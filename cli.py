...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    # a295 fix: was hard-coded localhost default which contradicts
    # app.py:52's local-mode default of `https://searx.be`. The
    # doctor reported "unreachable" on a fresh install while the
    # actual `huntova hunt` would succeed using searx.be — misleading
    # verdict. Now mirrors app.py: searx.be in local mode (since
    # cmd_doctor sets APP_MODE=local at line 2074), localhost in cloud.
...
def cmd_version(args: argparse.Namespace) -> int:
    print(VERSION)
    # a321: also surface update info here so `huntova --version` is a
    # quick way for any user to check if they're behind. The check is
    # cached upstream (6h TTL) so this stays fast and doesn't hammer
    # GitHub on repeat `--version` calls.
    try:
        avail, latest = _is_update_available()
        if avail and latest:
            try:
                from tui import bold, dim, yellow, cyan
            except ImportError:
                bold = dim = yellow = cyan = (lambda s: s)
            tag = (latest or "").lstrip("v")
            print(f"{yellow('▲')} update available: "
                  f"{dim(VERSION)} → {bold(tag)}  "
                  f"run {cyan('pipx upgrade huntova')}")
    except Exception:
        pass
    return 0
...
def cmd_recipe(args: argparse.Namespace) -> int:
    ...
    if sub == "import-url":
        ...
        if not url:
            ...
            return 1
        # a424 fix (BRAIN-63): SSRF guard. Pre-fix this command did
        # urllib.request.urlopen on the raw URL with no scheme check
        # (file://, gopher://, ftp:// all accepted) and no host check
        # (localhost, 169.254.169.254 cloud-metadata, RFC1918 internal,
        # link-local all reachable). A user copy-pasting a malicious
        # `huntova recipe import-url file:///etc/passwd` from a hostile
        ...
        _pu = _urlparse(url)
        if _pu.scheme.lower() not in ("http", "https"):
            print(f"[huntova] only http(s) URLs accepted (got {_pu.scheme!r})", file=sys.stderr)
            return 1
        if not _pu.netloc:
            print(f"[huntova] missing host in URL", file=sys.stderr)
            return 1
        # Reuse app.classify_url (private/loopback/link-local/reserved
        # IP rejection + DNS-rebinding via getaddrinfo). It returns "ok"
        # only for public-resolvable http(s) URLs.
        try:
            from app import classify_url as _classify
            _verdict = _classify(url)
            if _verdict != "ok":
                print(f"[huntova] refusing fetch — URL classified {_verdict!r} (private/loopback/blocked).", file=sys.stderr)
                return 1
        except ImportError:
            # Fallback minimal check if app isn't importable in this
            # context (shouldn't happen in normal CLI flow).
            host = _pu.hostname or ""
            if host.lower() in ("localhost", "metadata.google.internal", "instance-data") \
               or host.startswith("127.") or host == "0.0.0.0" or host.startswith("169.254."):
                print(f"[huntova] refusing fetch — host {host!r} is internal/loopback.", file=sys.stderr)
                return 1
        ...
        try:
            ...
            # a424: use a NoRedirectHandler + custom opener so a 30x
            # response doesn't bypass our host check by redirecting from
            # an allowed public host to a private destination.
            class _NoRedirect(urllib.request.HTTPRedirectHandler):
                def redirect_request(self, req, fp, code, msg, headers, newurl):
                    raise urllib.error.HTTPError(
                        req.full_url, code,
                        f"redirect to {newurl} blocked (SSRF guard)",
                        headers, fp,
                    )
...
# BRAIN-CLI-1 (a800): friendlier argparse error UX. Three things:
#   1. `huntova hutn` now suggests "Did you mean: hunt?" instead of dumping
#      the 40+ choices list.
#   2. `huntova hunt --bogus-flag` shows `huntova hunt`'s narrow usage line
#      instead of the top-level usage with every subcommand listed.
#   3. Both behaviours route through the same `_HuntovaArgumentParser`
#      subclass — no two-stage parser refactor required for the existing
#      ~7000-line build_parser body.
#
# Implementation note: argparse's error-message text ("invalid choice",
# "unrecognized arguments") IS surface area we depend on, but only as
# detection. We never reformat the canonical message — we *replace* it
# with a Huntova-team-voice line, then preserve argparse's exit-code 2
# convention. If the message format ever changes between Python minors
# we silently fall back to argparse's stock output (see _safe_match).
import difflib as _difflib


def _safe_match(prefix: str, message: str) -> bool:
    try:
...
class _HuntovaArgumentParser(argparse.ArgumentParser):
    """argparse subclass with Huntova-flavoured error messages.

    - Invalid subcommand → suggest closest match via difflib + a 1-line
      "try `huntova --help`" hint, instead of the giant choices dump.
    - Unrecognized flags after a known subcommand → re-route to that
      subparser's narrow usage line so users see only the flags that
      apply to the command they typed.
    """

    def error(self, message: str) -> None:  # type: ignore[override]
        # 1. Bad subcommand at the top level. argparse formats this as
        #    `argument <dest>: invalid choice: 'x' (choose from ...)`.
        if _safe_match("argument cmd: invalid choice:", message):
            try:
                bad = message.split("'")[1]
            except (IndexError, ValueError):
                bad = ""
            choices = list(getattr(self, "_huntova_choice_names", []))
            suggestion = _difflib.get_close_matches(bad, choices, n=1, cutoff=0.55)
            sys.stderr.write(f"huntova: unknown command: {bad!r}\n")
            if suggestion:
                sys.stderr.write(f"  did you mean: {suggestion[0]}?\n")
            sys.stderr.write("  run `huntova --help` for the full list.\n")
            self.exit(2)
            return  # pragma: no cover — exit() raises SystemExit

        # 2. Unrecognized flag after a known subcommand. argparse
        #    formats this as `unrecognized arguments: --foo --bar`.
        if _safe_match("unrecognized arguments:", message):
            sub_argv = list(getattr(self, "_huntova_last_argv", []) or [])
            chosen = next((a for a in sub_argv
                           if not a.startswith("-")
                           and a in getattr(self, "_huntova_choice_names", [])),
                          None)
            if chosen:
                sp = getattr(self, "_huntova_subparsers", {}).get(chosen)
                if sp is not None:
                    sp.print_usage(sys.stderr)
                    sys.stderr.write(
                        f"huntova {chosen}: error: {message}\n"
                        f"  run `huntova {chosen} --help` for the supported flags.\n"
                    )
                    self.exit(2)
                    return  # pragma: no cover
            # fall through to default error formatting
        super().error(message)


def build_parser() -> argparse.ArgumentParser:
    p = _HuntovaArgumentParser(
        prog="huntova",
    ...
    # BRAIN-CLI-1: capture choice names + subparser map on the parser so
    # `_HuntovaArgumentParser.error()` can route to the right subparser
    # for unrecognized-flag errors and produce difflib suggestions for
    # typo'd subcommands. Wrapped in try/except so any internal argparse
    # shape change (private API) silently falls back to default errors.
    try:
        p._huntova_choice_names = tuple(sub.choices.keys())  # type: ignore[attr-defined]
        p._huntova_subparsers = dict(sub.choices)  # type: ignore[attr-defined]
...
def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    # BRAIN-CLI-1: stash argv so the error handler can identify which
    # subcommand the user was trying to invoke (for narrow-usage errors).
    try:
        parser._huntova_last_argv = list(argv) if argv is not None else list(sys.argv[1:])  # type: ignore[attr-defined]
    except Exception:  # pragma: no cover
        pass
    try:
        args = parser.parse_args(argv)
    except KeyboardInterrupt:
        # BRAIN-CLI-2: parser may swallow Ctrl+C during interactive prompts
        # before the main try/except catches it. POSIX SIGINT exit code
        # is 128+2=130. Keep the contract terminal users expect.
        sys.stderr.write("\n")
        return 130
    if not args.cmd:
...
if __name__ == "__main__":
    # BRAIN-CLI-2 (a800): graceful Ctrl+C. Without this, hitting Ctrl+C
    # mid-`huntova hunt` / `huntova chat` / etc. dumps a Python traceback.
    # POSIX convention: exit code 128+SIGINT(2)=130.
    try:
        sys.exit(main())
    except KeyboardInterrupt:
        sys.stderr.write("\n")
