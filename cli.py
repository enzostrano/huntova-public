...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    if not has_key:
        print("[huntova] no API key configured — run `huntova onboard` first "
              "or set HV_ANTHROPIC_KEY (default) / HV_GEMINI_KEY / HV_OPENAI_KEY")
    ...
    if not args.no_browser:
        # First-run users with no key configured land on /setup so they
        # see the wizard immediately. Returning users open the dashboard.
        # `setup_first=True` (passed by `huntova onboard --browser`) forces
        # the wizard regardless of key state.
        force_setup = bool(getattr(args, "setup_first", False))
        landing = "/setup" if (force_setup or not has_key) else "/"
        try:
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if sub == "unset":
        # Delete a key from config.toml. Mirrors `openclaw config unset`.
    ...
    if sub == "validate":
        ...
        # type checks. Pattern adapted from openclaw config validate.
        if not cfg_path.exists():
...
def cmd_onboard(args: argparse.Namespace) -> int:
    ...
    """
    # Reset before the wizard runs (pattern from openclaw/openclaw,
    # specifically src/commands/onboard.ts — fresh Python implementation,
    # no TS code reproduced).
    ...
    # OpenClaw-parity safety gate: --no-prompt requires --accept-risk so
    # users running scripted setups make a deliberate ack of the
    # autonomy + network-access surface area.
...
def _apply_provider_flags(args: argparse.Namespace) -> None:
    ...
    Mirrors OpenClaw's per-provider --*-api-key flag pattern. Maps each
    `--<slug>-api-key` to its HV_*_KEY env var and writes via
    secrets_store.set_secret. Best-effort — failures are surfaced but
    ...
    if saved:
        print(f"[huntova] saved {len(saved)} key(s) from flags: {', '.join(saved)}")
...
def _apply_reset_scope(scope: str) -> None:
    ...
    Pattern adapted from openclaw/openclaw's ResetScope union type.
...
def _onboard_v2(args: argparse.Namespace) -> int:
    ...
    # ── Existing-config card (OpenClaw-style) ─────────────────────
    ...
    _tips = [
        ...
        ("huntova examples ls",        "4 starter recipes for common ICPs"),
...
def cmd_security(args: argparse.Namespace) -> int:
    ...
    Ports the lead-gen-relevant subset of OpenClaw's `security *` checks.
    ...
    # 3. plaintext secrets file present at all
    plain = cfg_dir / "secrets.json"
    if plain.exists():
        add("plaintext secrets.json", 1, "warn",
            f"{plain} exists — keychain fallback engaged",
    ...
    for ln in text.splitlines(keepends=True):
        ...
        if in_block:
            if stripped == _RC_FENCE_CLOSE:
                ...
                continue
            # Backwards-compat: pre-fence-close installs (v0.1.0a10)
            # have only the open fence + 1 line. Stop after that 1
            # line to avoid eating extra user content.
            if not saw_close_fence:
                # Drop exactly one body line then leave block.
                in_block = False
                continue
        out_lines.append(ln)
...
# Pattern adapted from OpenClaw's `openclaw recipe export|import` —
...
def cmd_chat(args: argparse.Namespace) -> int:
    ...
    Modeled on OpenClaw's `openclaw chat` (which is `openclaw tui --local`).
    ...
    SYSTEM_PROMPT = (
        ...
        "contact_email, why_fit). When unsure or the user asks a "
        "how-to / definition / status question, use action=answer."
    )
    ...
    def _ask_ai(history: list[dict]) -> dict | None:
        ...
        if action not in ("start_hunt", "list_leads", "answer"):
            print(f"  {red('●')} Unknown action {action!r} (expected start_hunt / list_leads / answer).")
            return None
        return parsed
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ob.add_argument(
        ...
        help=(
            ...
            "full=erase config + keychain + local DB. Pattern adapted from OpenClaw."
        ),
    )
    # OpenClaw-style flow / safety / output flags. --flow picks the
    ...
    # with --no-prompt (mirrors OpenClaw's non-interactive policy).
    ...
    # Per-provider keys for scripted setup (mirrors OpenClaw's
    ...
    # from OpenClaw. Implementation lives in cli_memory.py to keep this
    ...
    # pattern adapted from OpenClaw `openclaw migrate`. Implementation
    # lives in cli_migrate.py for the same size-cap reason.
    try:
    ...
    # debugging a hunt. Pattern adapted from OpenClaw `openclaw logs` /
    # `openclaw tail`. Implementation lives in cli_logs.py.
    try:
    ...
    # quality benchmark (no quota burn). Pattern adapted from OpenClaw
    # `openclaw bench`. Implementation lives in cli_benchmark.py.
    try:
    ...
    # OpenClaw-style grouped `--help` output. argparse's default subparser
...
_HELP_CATEGORIES: list[tuple[str, tuple[str, ...]]] = [
    ("Getting started", ("onboard", "doctor", "status", "version")),
    ("Daily use",       ("serve", "chat", "hunt", "ls", "lead", "memory",
                          "teach", "history", "security")),
    ("Outreach",        ("approve", "outreach", "examples", "recipe",
                          "share", "migrate")),
    ("Plugins / customization", ("plugins", "config")),
    ("Daemon / ops",    ("daemon", "test-integrations", "logs", "metrics",
                          "cloud", "benchmark")),
