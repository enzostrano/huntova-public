...
- **No analytics, telemetry, or crash reports are sent by default**. The local CLI has zero outbound network calls outside (a) your AI provider, (b) your configured SearXNG instance, (c) the URLs the agent fetches during a hunt, and (d) optional Playwright/Jina fallbacks for JS-heavy pages. Anonymous usage telemetry is **opt-in only** via `huntova telemetry enable` — when enabled, the CLI sends a small set of event names (`cli_init`, `cli_hunt`, `try_submit`) plus platform + version. **No keys, no queries, no lead data, no PII is ever shipped.** Disable any time with `huntova telemetry disable`. The endpoint is configurable via `HV_METRICS_URL`.
