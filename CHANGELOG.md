...
## 0.1.0a920 — May 4 2026 — CLI argparse UX hardening (BRAIN-CLI-1 + BRAIN-CLI-2)

Wave-2 swarm CLI audit. Two real argparse UX bugs fixed and pinned by 10 new regression tests. Bundled per the batched-release standing order — one tag instead of one tag per fix.

### Bug fixes

1. **BRAIN-CLI-1 — typo'd subcommand dumped a 40+ choices blob, no `did you mean`.** `huntova hutn` (top-level argparse `error()` path) used to print `argument cmd: invalid choice: 'hutn' (choose from serve, tail, run, init, status, config, test-integrations, daemon, onboard, doctor, security, version, update, chat, hunt, ls, …)` — a 4-line wall of text with every registered subcommand inlined into one wrapped paragraph. New users couldn't tell which one they meant. Fix: introduced `_HuntovaArgumentParser` (subclass of `argparse.ArgumentParser`) at `cli.py:6710`. Its `error()` override detects `argument cmd: invalid choice:` messages and replaces them with `huntova: unknown command: 'hutn'` + `did you mean: hunt?` (when `difflib.get_close_matches(bad, choices, cutoff=0.55)` finds a near match) + `run \`huntova --help\` for the full list.` Random garbage like `huntova zzzzzz` lands cleanly with no false suggestion.

2. **BRAIN-CLI-1 (companion) — unknown flag on a known subcommand printed the TOP-level usage line.** `huntova hunt --bogus-flag` used to print the `usage: huntova [-h] [--version] {serve,tail,run,…40 names…} ...` blob followed by `error: unrecognized arguments: --bogus-flag`. The "unrecognized arguments" error is raised by argparse on the *root* parser after the subparser action returns leftovers, so by the time `error()` runs we're back at the top level. Fix: same `error()` override detects `unrecognized arguments:` + walks `_huntova_last_argv` (stashed in `main()` before `parse_args`) to find which subcommand the user typed, then routes the error through that subparser's `print_usage()` so users see only `usage: huntova hunt [-h] [--countries …] [--max-leads …] …` — the flags that actually apply.

3. **BRAIN-CLI-2 — Ctrl+C dumped a Python traceback at the entry point.** `cli.py` end-of-file `sys.exit(main())` had no `KeyboardInterrupt` guard. Hitting Ctrl+C during any subcommand (most commonly `huntova chat`, `huntova hunt`, `huntova onboard`) printed `Traceback (most recent call last): … KeyboardInterrupt` to a freshly-onboarded user's terminal. Fix: wrapped the entry point + the inner `parse_args` call in `try/except KeyboardInterrupt` returning exit code 130 (POSIX 128 + SIGINT(2) convention) with a clean `\n` to stderr.

### Test additions

- `tests/test_cli_argparse_ux.py` — 10 new tests covering: typo suggestion (close + far + no-match), unknown-flag-on-subcommand routing for two different subcommands, top-level vs subcommand `--version` parity, parser internal metadata (`_huntova_choice_names` / `_huntova_subparsers`), grouped-help layout preservation, and `KeyboardInterrupt` → exit 130 at the `main()` boundary.
- Full suite: 883 tests passing locally.

### Files touched

- `cli.py:25` — `VERSION` bump 0.1.0a620 → 0.1.0a920.
- `cli.py:6710` — new `_HuntovaArgumentParser` subclass + `_safe_match` helper.
- `cli.py:6789` — `build_parser()` now instantiates `_HuntovaArgumentParser` instead of `argparse.ArgumentParser`.
- `cli.py` end of `build_parser` — populate `_huntova_choice_names` + `_huntova_subparsers` after all subparsers register (defensive try/except — argparse private API).
- `cli.py` `main()` + `__main__` guard — stash argv before `parse_args` and catch `KeyboardInterrupt` → 130.
- `pyproject.toml:7` — version mirror bump.
- `tests/test_cli_argparse_ux.py` — new file, 10 tests.

### Other CLI checks the audit cleared

- `huntova` (no args) — already smart-defaults via `_smart_default_cmd` (onboard / quickstart / serve depending on state). No bug.
- `huntova --help` — grouped category renderer (`_attach_grouped_help`) intact. Still surfaces every subcommand including the bundled `cli_*.py` ones (memory, migrate, approve, logs, inbox, sequence, pulse, schedule, remote, teach, benchmark) registered via the `register(sub)` pattern.
- `huntova doctor`, `huntova security`, `huntova pulse`, `huntova schedule print`, `huntova status` all produce useful text on a fresh local install — no unfriendly stack traces.
- `huntova inbox` / `huntova sequence` / `huntova recipe` correctly emit argparse `the following arguments are required: <subcmd>` with the subcommand-specific usage line — already the desired behaviour.
