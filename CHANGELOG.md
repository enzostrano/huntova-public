...
## 0.1.0a508 — May 4 2026 — Sidebar pill claimed "Leads 1" but the Leads view rendered "No leads yet" because `/api/leads` always returned a bare JSON array (`return leads` at server.py:5302) while every reader unwrapped `(d && d.leads) || []` — `.leads` on an array is `undefined`, so the empty-fallback fired for every user with ≥1 row in the leads table; sidebar pill (sourced from `/api/status` → `db.get_leads_count` SQL `COUNT(*)`) and main view contradicted each other and masked real lead deliveries; both readers in templates/jarvis.html (loadLeads + lead-detail drill-in) now coerce array-or-`{leads:[…]}` shapes without a contract break (BRAIN-PROD-3)

### Bug fix (BRAIN-PROD-3, /api/leads response-shape mismatch)

`/api/leads` returns a bare JSON array — see the
final line of the handler at server.py:5302
(`return leads`). The two readers in
`templates/jarvis.html` (the main `loadLeads()`
fetch + the lead-detail drill-in refetch) both
extracted leads with `(d && d.leads) || []`. On a
JSON array `d.leads` is `undefined`, so the
fallback to `[]` fired every single time and
`_leads` was always empty.

The user-visible contradiction:

- The sidebar pill `pillLeadsVal` reads
  `total_lead_count` from `/api/status`, which
  routes through `db.get_leads_count(uid)` — a
  single SQL `COUNT(*)`. With one persisted lead
  the pill correctly shows "1".
- The main Leads view + nav badge use `_leads`,
  which the broken reader kept at `[]`. The view
  rendered "No leads yet" and the crumb said "0
  found" even though the row existed.

Net effect: a real lead delivery was invisible to
the user from the dashboard. They had to open the
SSE feed or hit the DB directly to confirm the
agent had actually saved anything. This was the
source of the "Leads says 1 lead but untrue"
report — the truth was that `total_lead_count` was
right, the leads-list reader was wrong.

#### Why a defensive shape coercion (and not a server-side wrap)

The cleanest server-side fix would be to wrap the
response in `{"leads": [...]}`. That matches the
contract the JS already expected. We didn't take
that route because:

1. `/api/leads` has shipped as a bare array since
   the endpoint existed. Any external consumer
   (CLI, tests, future programmatic use) that
   trusts the documented `list[Lead]` shape would
   break on a wrap.
2. The frontend was the only broken caller, and a
   single grep confirmed there were exactly two
   call sites. A 1-line defensive coercion is
   safer than churning the API contract.

Both readers now do:

```javascript
const _arr = Array.isArray(d) ? d : ((d && Array.isArray(d.leads)) ? d.leads : []);
_leads = _arr.slice();
```

— which keeps array-shape working today and
silently absorbs a future `{leads:[…]}` rewrap if
we ever choose to make one.

#### Regression test

`tests/test_api_leads_response_shape_contract.py`
asserts the contract that the frontend reader now
honours, with three guards:

1. The handler returns the bare list (or a wrap
   that contains it under `.leads`) — the loose
   form so a future server-side wrap doesn't break
   the test.
2. The two reader call sites in
   `templates/jarvis.html` use the
   array-or-`.leads` coercion (regex-anchored on
   the `Array.isArray(d)` ternary).
3. The broken `(d && d.leads) || []` pattern is
   gone from the file. Without this guard a refactor
   could re-introduce the unwrap and the bug
   silently returns.

### Files touched

- `templates/jarvis.html` — both `/api/leads`
  readers (loadLeads at the Leads view + the
  lead-detail drill-in refetch) coerce array OR
  `{leads:[…]}` and add a comment block that
  documents the symptom + endpoint contract so a
  future contributor doesn't reintroduce the
  unwrap.
- `tests/test_api_leads_response_shape_contract.py`
  — new file. Three guards: server still emits a
  list-shape (or a wrapped `leads:[…]`), both
  readers do shape coercion, the broken
  `(d && d.leads) || []` pattern is gone.
