...
## v0.1.0a2048 -- May 10 2026 -- Re-seal pytest baseline + auth-error generic-ification + persistence-error visibility + Makefile

Three-agent audit: pytest-failure root-cause, full-codebase code review, OSS layout analysis. The Python-package restructure ("`huntova/` package layout") was deferred — too high-risk for a single session given 200+ test files reference bare module names. Everything below is the safe-to-apply subset.

### Bug fixes -- preflight / dry-run

- **`huntova hunt --dry-run` rejected with "ICP profile missing" even though dry-run is documented as a wiring smoke test that doesn't touch the lead pipeline** (`preflight.py:_requirements_for`). The dry-run relaxation block already dropped SMTP for `sequence_run` / `outreach_send`; now it also drops `ICP_PROFILE` for `hunt`. Aligns with the comment at `cli.py:5798`: "--dry-run skips the key requirement — pure wiring smoke test." Restores `tests/test_cli.py::test_hunt_dry_run_*` from the post-a2046 broken baseline.

### Bug fixes -- inbox auto-reply detection

- **`_is_autoreply` flagged genuine human replies as bounces when their headers omitted Return-Path** (`cli_inbox.py:_is_autoreply`). The bounce-signal check `_rp in ("", "<>", "< >")` ran unconditionally; missing headers normalised to `""` and tripped the gate. Most legitimate IMAP replies don't carry a Return-Path on the way back. Now the empty/null-shape check only runs when the header is actually present in the dict. Restores `test_cli_inbox_audit::test_is_autoreply_genuine_reply_not_flagged` and `test_is_autoreply_auto_submitted_header`.

### Bug fixes -- auth / changelog endpoint info-leak hardening

- **`/auth/login` echoed `str(ValueError)` to clients on bad credentials** (`server.py:auth_login`). Login error messages distinguishing "unknown email" from "wrong password" enable account enumeration. Now login surfaces a single generic "Invalid credentials" 401 to the client and logs the specific reason to stderr for ops.
- **`/api/changelog/release-notes` echoed raw exception text on read failure** (`server.py:13656`). Pathlib / IO exceptions can include absolute server paths. Now logs the actual error to stderr server-side and returns a generic "Unable to read changelog" envelope to clients.

### Bug fixes -- agent persistence visibility

- **`save_seen_history` and `save_domain_blocklist` swallowed every exception silently** (`app.py:652, 664`). Disk-full / permission errors / encoding mismatches dropped de-dup state and domain-block state with zero diagnostic. Now both call `emit_log(..., "warn")` on failure so the user sees the problem in `huntova logs daemon` / dashboard log stream.

### Tests -- baseline re-sealed

After fixing the production bugs above plus three Windows-platform test issues:

- **`test_save_config_sets_0600` and `test_derive_key_creates_salt_file_on_first_use`** wrap `os.chmod(0o600)` assertions which NTFS doesn't enforce; both gain `@pytest.mark.skipif(sys.platform == "win32")`. The POSIX behaviour is still tested on the Linux/macOS CI cells the canonical pre-launch smoke gate runs.
- **`test_wizard_scan_autoadvance::*` (3 tests)** read `templates/jarvis.html` via `pathlib.Path.read_text()` which on Windows defaults to cp1252 and fails on the file's UTF-8 byte 0x90. Added `encoding="utf-8"` so the test runs on every platform.

**Pytest baseline: 1724 passed, 6 skipped, 0 failed.** Re-seals the broken-since-a2013 baseline that every release between a2013 and a2047 marked "pending re-run".

### Tooling

- **Added `Makefile`** with conventional dev targets: `install`, `install-dev`, `test`, `test-fast`, `lint`, `build`, `release-check`, `clean`. `lint` runs the AST + CSS brace-balance checks called out in the release workflow. Brings the repo's CLI ergonomics in line with what contributors expect from a polished open-source Python project.

### What was NOT done this release (deferred)

- **Full Python package restructure** to `src/huntova/` with subsystem grouping (core/, db/, auth/, etc.). Reason: 200+ test files reference bare module names (`from app import X`, `from db import Y`); a re-export shim in `__init__.py` is feasible but needs its own session for thorough wheel + install + pytest validation across all platforms. Tracked as a follow-up.
- **Changelog archival** of pre-a2000 entries to `docs/CHANGELOG-archive.md`. Considered, deferred — the 484KB CHANGELOG is large but search-friendly as one file; archival is a debatable trade-off and not a clear win.
- **Other code-review findings** (heartbeat-loop stop event, async/sync mixing in `app.py`'s nested `loop.run_until_complete`, broader silent-failure scrub across server.py) — these are real but their blast radius requires careful targeted commits, not a sweep. Tracked as follow-ups.


---

## v0.1.0a2047 -- May 10 2026 -- Public-repo polish: scrub internal references, surface AGPL source link, telemetry honesty, Code of Conduct

Five-agent audit sweep across the working tree. No new features; no behavior changes; everything below is hygiene for a public open-source distribution.

### Repo / privacy

- **Scrubbed maintainer first-name from operational comments** in `server.py` (cloud-proxy + recipe-registry beta-flag headers), `docs/workflows/pre-launch-smoke.yml` (CI trigger doc), `tests/test_settings_clear_resets_to_default.py` (failure-mode docstring), `tests/test_tui_audit.py` (no-AI-mentions test rationale). Author attribution in `LICENSE`, `pyproject.toml`, and `TERMS.md` (trademark notice) is retained — those are intentional legal/copyright signals. The casual `Enzo flips the flag` / `(Enzo, 2026-05-04…)` style was leaking the impression of a one-person internal-team workflow into source code that ships publicly.
- **Scrubbed account-specific Hostinger preview subdomain** from `CHANGELOG.md` (entries v0.1.0a325 + v0.1.0a164 history) and `templates/landing.html` (a325 self-healer comment block). The static-preview URL is an account-bound identifier that doesn't belong in a public repo even though the deploy itself is public.
- **Generic-ified two placeholder examples** (`server.py` `update_settings` JSON example + `templates/jarvis.html` email-signature placeholder) from `Enzo` to `Your Name` so the example is unambiguously a template, not a personal default.

### Legal / AGPL hygiene

- **Added `CODE_OF_CONDUCT.md`** (Contributor Covenant 2.1, adapted) with reporting channel pointing at the GitHub Security Advisory page. Standard for AGPL projects accepting plugin / issue / PR contributions.
- **Added AGPL §13 source-availability link to the dashboard** (`templates/jarvis.html` sidebar corner). Section 13 of AGPL-3.0 requires that users interacting with the software via a network give users prominent access to the source. The marketing landing page + setup wizard already linked GitHub; the main dashboard `jarvis.html` did not. The new corner reads `huntova · vX · source` where `source` is a target=_blank link to the public repo. The sibling version span keeps the same auto-update behavior; the source link is permanent.
- **Updated `PRIVACY.md` to honestly document the opt-in telemetry feature**. The existing copy said "No analytics, telemetry, or crash reports are sent" — true by default, but the CLI does ship an opt-in `huntova telemetry enable` path (event names + platform + version, no keys/queries/PII). The new copy makes the default-off behavior explicit AND discloses what the opt-in path sends, enable/disable commands, and the `HV_METRICS_URL` override. Aligns the privacy policy with what the code actually does.

### Files changed
- `server.py` (3 comment scrubs + 1 placeholder generic-ification)
- `templates/jarvis.html` (sidebar corner source link + version-span split + email-signature placeholder)
- `templates/landing.html` (a325 comment block scrub)
- `docs/workflows/pre-launch-smoke.yml` (CI trigger comment scrub)
- `tests/test_settings_clear_resets_to_default.py` (docstring scrub)
- `tests/test_tui_audit.py` (docstring scrub)
- `PRIVACY.md` (telemetry disclosure)
- `CODE_OF_CONDUCT.md` (new)
- `CHANGELOG.md` (this entry + 2 historical scrubs)
- `cli.py` + `pyproject.toml` (version bump)

### Tests

Pytest baseline pending re-run. Pre-existing baseline failure on `test_hunt_dry_run_json_emits_valid_jsonl` (rc=2 — `local_env` fixture doesn't seed an ICP that the dry-run path now requires) is unrelated to this change set; deferred to a follow-up.
...
**Per-lead LinkedIn outreach pack endpoint** (`server.py /api/leads/{lead_id}/linkedin_pack`, backed by `app.py:_generate_linkedin_pack`).

For any saved lead, the user can now request a copy-paste-ready LinkedIn outreach package that mirrors how a senior B2B operator actually opens a cold relationship at exec level — short, specific, no pitch, language matched to the prospect's working language. Returns:

- `decision_maker_role` — best-guess buyer role title in the prospect's language (e.g. "Head of Investor Relations", "Direttore Comunicazione", "Directrice Communication")
- `decision_maker_name` — verbatim name from the source page text, or `null` (never fabricated)
- `linkedin_search_url` — always a `linkedin.com/search/results/people/?keywords=…` URL (never an invented profile slug — the AI cannot reliably know the slug)
- `working_language` — `en | it | fr | de | es | pt`, detected from page content / company HQ / role-title conventions
- `connection_request_message` — ≤290 chars, peer-to-peer tone, in `working_language`, anchored on a concrete observation (event, recent post, initiative)
- `follow_up_dm` — 200–500 chars, what the user sends 3 days after the connection is accepted, still no pitch
- `verifiable_observation` — the concrete fact the hook is anchored on, with a quote or page-section reference

This is intentionally distinct from the existing `linkedin_note` field (which is a cold-pitch accessory generated alongside the rewritten email). The pack is LinkedIn-native — peer-to-peer connection-request → DM sequence — not email-style copy.

### Hard rules enforced server-side (the AI is never trusted)

- LinkedIn URLs are forced to the `/search/results/people/?keywords=…` form — invented profile slugs are detected and replaced with a search URL built from `company + role + name` via `_linkedin_search_url_for`.
- Connection requests are truncated to 290 chars at a word boundary if the AI overshoots the cap.
- `working_language` is clamped to the allowed set, defaulting to `"en"` for anything unknown.
- Placeholder names like `""`, `"null"`, `"None"`, `"N/A"` are normalised to `None` so the frontend never displays "Hi None,".
- `follow_up_dm` is hard-capped at 500 chars.
- AI-call failure or non-JSON response returns a structured `{"error": ..., "linkedin_search_url": ...}` so the user still gets a usable search URL even when the model is unavailable.

Persisted on the lead via atomic `db.merge_lead` (same lost-update guard as `api_research`) so concurrent dashboard writes don't clobber the pack.

### Files
- `app.py` — `_LINKEDIN_PACK_SYSTEM` prompt + `_linkedin_search_url_for` + `_validate_linkedin_pack` + `_generate_linkedin_pack`.
- `server.py` — `POST /api/leads/{lead_id}/linkedin_pack` endpoint with `require_feature("research")` gate, BYOK provider check, atomic merge persistence.
- `templates/jarvis.html` — "🤝 LinkedIn pack" button on the lead-detail email-actions row + result panel rendering decision-maker / search-button / connection-request / follow-up-DM / language-badge / verifiable-anchor / validator-warnings, with copy-to-clipboard on each text field. Hydrates from `lead.linkedin_pack` on initial render so the panel survives view-switches.
- `tests/test_linkedin_pack.py` — 46 tests covering URL builder, char caps, language clamp, name normalisation, AI-failure paths, end-to-end validator behaviour.
- `cli.py` + `pyproject.toml` + `CHANGELOG.md`.

### Tests
1684 prior + 46 new = **1730 passing**. No regressions.
...
- **Deployed marketing site** (a163) — live as a static preview deploy for share-while-pre-domain demos.
