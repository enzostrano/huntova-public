...
## 0.1.0a533 — May 4 2026 — `/api/memory` (POST manual write + DELETE archive) was on the legacy default `ai` bucket with a bare `JSONResponse(status_code=429)` and no body byte-cap — rogue scripts could push multi-MB payloads that feed the chat SYSTEM_PROMPT and burn the shared chat/wizard budget; new `memory` bucket (60s/20) + full BRAIN-91/112/113/117 contract (BRAIN-145)

### Bug fix (BRAIN-145, /api/memory adjacent-AI-surface parity)

`POST /api/memory` accepts a user-authored manual
memory entry (`{key, value}`) which is then surfaced
back into the chat SYSTEM_PROMPT for grounded replies.
The pre-fix handler had:

- A legacy default-bucket `_check_ai_rate(user["id"])`
  call (no bucket arg → `ai` default). Manual memory
  writes therefore competed with chat / wizard for the
  same budget.
- A bare `JSONResponse({...}, status_code=429)` on
  burst block — no `Retry-After`, no
  `RateLimit-Limit/Remaining/Reset`, so clients had no
  IETF backoff signal.
- No `_enforce_body_byte_cap(...)` before
  `request.json()`. A multi-MB JSON body would be
  parsed into memory before any bound was checked.
  Memory rows are surfaced into the chat
  SYSTEM_PROMPT, so unbounded bodies are both a
  table-bloat vector and a prompt-injection-poisoning
  vector.
- No `_attach_burst_rate_headers(...)` on success →
  no proactive client-side budget signal.

`DELETE /api/memory/{memory_id}` (the archive
side-channel) had no rate-limit at all. A spam-archive
loop has no upper bound.

Per Huntova engineering review on adjacent-mutating-
endpoint parity (BRAIN-122/139/142/144): every
mutating endpoint that triggers DB writes or feeds
prompt-construction state must enforce the same
front-door guarantees as the wizard surface —
bounded body size, per-endpoint rate limit, IETF
RateLimit-* headers on both 429 and success.

Fix: new `memory` bucket in `_RATE_BUCKETS`
(60s/20 — interactive write cadence, generous enough
that legitimate dashboard use never trips it). Both
handlers migrated to the standard pattern:

- `_check_ai_rate(user_id, bucket="memory")` →
  `_rate_limit_429(...)` on burst block.
- `_attach_burst_rate_headers(response, user_id, "memory")`
  on success.
- `_enforce_body_byte_cap(...)` on the POST path
  (DELETE has no body).
- `response: Response` parameter added to both
  signatures so FastAPI injects the mutable
  response object.
- POST + DELETE share the same `memory` bucket so a
  rogue client can't burn through writes via the
  archive side-channel.

7 new regression tests in
`tests/test_memory_endpoints_hardening.py`.

786 / 786 tests passing.

### Files

- `server.py`: new `memory` bucket in `_RATE_BUCKETS`. `api_memory_record` + `api_memory_archive` handlers migrated to the BRAIN-91/112/113/117 contract.
- `tests/test_memory_endpoints_hardening.py`: new — 7 tests guarding the parity contract.
