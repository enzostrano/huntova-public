...
## v0.1.0a910 — 2026-05-04 search abstraction + retry policy audit

Search-pipeline robustness sweep on `app.py`. Three real-bug fixes
batched per the release-cadence standing order.
...
- **SearXNG 5xx silently swallowed (no retries, no SSE warning).**
  ...
  through the bare `except Exception` and the agent went straight to
  the DDG fallback after a single attempt. A flaky self-hosted
  SearXNG burned the entire hunt's quota by hammering DDG (which has
  its own rate limits, so cascading failures dropped lead volume to
  zero with no signal to the user). Now `_searxng_query_once`
  classifies every HTTP outcome (`http_5xx` / `http_4xx` /
  `timeout` / `conn` / `decode`), `search()` retries retryable
  failures within a per-query budget (`SEARXNG_MAX_RETRIES_PER_QUERY`,
  default 2) with exponential backoff + jitter, and surfaces a clear
  per-failure SSE log line. (`app.py:5142+5263`)
- **No per-hunt circuit breaker.** Every query independently retried
  even when SearXNG had been down for 30 consecutive queries — wasted
  RTTs + wasted DDG quota. Added `_SearxngBreaker`: after
  `SEARXNG_MAX_CONSEC_FAILURES` consecutive failures (default 3) the
  breaker trips and all subsequent calls skip the SearXNG round-trip
  entirely. A single user-facing SSE warning fires the first time the
  breaker trips so the user knows the search backend degraded
  (previously: "zero leads" with no explanation). `reset_search_breaker()`
  clears state at hunt start so a previous hunt's failures don't leak.
  (`app.py:5142`)
- **`run_agent()` refused to start when `check_searxng()` returned
  False.** Self-inflicted outage — `search()` had a working DDG
  fallback all along. Now a failed startup probe pre-trips the
  breaker (so the per-query retry budget isn't paid up front) and
  emits one degraded-mode warning, then continues. (`app.py:6707`)

### Other

- 22 new regression tests in `tests/test_search_retry.py` covering
  the retry budget, breaker tripping/resetting/single-warning, 4xx
  no-retry, 429-as-retryable, JSON-decode classification, and result
  capping. Tests stub `_search_session.get` so no network is hit.
