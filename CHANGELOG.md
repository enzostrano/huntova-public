...
## 0.1.0a730 — May 4 2026 — Leads board: count-vs-list desync fix ("1 lead but list empty") (LEADS-1/2/3)

### Bug fixes
- **LEADS-1 — visible leads list always empty when DB has leads.** `templates/jarvis.html` `loadLeads()` and `openLeadDetail()` both read `_leads = ((d && d.leads) || []).slice()` from the `/api/leads` response, but the endpoint (`server.py` L5466 `return leads`) returns a BARE JSON array, not `{leads:[…]}`. So `d.leads` was always undefined, `_leads` always resolved to `[]`, and the list view was empty for every user with persisted leads — while the sidebar pill kept reading the truthful DB COUNT(*) from `/api/status total_lead_count`. Net user-visible bug: "the pill says 1 lead but the list is empty / untrue". Both call sites now use `Array.isArray(d) ? d : (d && Array.isArray(d.leads) ? d.leads : [])` so they tolerate both shapes without re-introducing the regression.
- **LEADS-2 — silent 300-row render cap misled the count badge.** `renderLeads()` did `.filter().slice(0, 300)` and reported the post-slice length as "N found", silently lying to anyone with >300 matches. Now splits the full match-set from the rendered window and labels truncation explicitly: `1247 of 1247 · showing first 300`.
- **LEADS-3 — SSE 'lead' event bumped the pill but not the list.** `_agentSse.addEventListener('lead', () => loadStatus())` only refreshed the count via `/api/status`. Users on the leads board watched the pill tick "+1" with no new row appearing until they manually clicked Refresh. The handler now also fires `loadLeads()` when `_currentView === 'leads'` so pill + list stay in lockstep without burning bandwidth on other views.

### Tests
- `tests/test_leads_count_surface.py` — 14 new regression tests pinning the canonical contracts:
  - `/api/leads` is GET-only and returns a bare JSON array (the frontend reader's primary branch).
  - `/api/status total_lead_count` is the int-typed source of truth for the pill.
  - `total_lead_count == len(/api/leads)` invariant holds across save / delete / SSE arrival paths.
  - `limit` is clamped to the documented HARD_CAP=10000 regardless of what the client sends.
  - Direct guard against the legacy `((d && d.leads) || []).slice()` reader pattern reappearing in `templates/jarvis.html`.
- 898 total tests pass (884 prior + 14 new).
