...
## 0.1.0a529 — May 4 2026 — extracted shared `_rate_headers_for(...)` builder; the three rate-limit-related helpers (`_rate_limit_429`, `_burst_rate_headers`, `_daily_quota_429`) now route IETF `RateLimit-*` / `Retry-After` header construction through one source of truth so the contract can no longer drift across the 429 + success paths (BRAIN-146)

### Refactor (BRAIN-146, third-order shared-contract extraction for rate-limit headers)

`server.py` exposes three rate-limit-related helpers
introduced by BRAIN-112 (a481) + BRAIN-113 (a482):

- `_rate_limit_429(...)` — burst-bucket 429 with
  `Retry-After: <window-seconds>` + the IETF triple
  (`RateLimit-Limit`, `RateLimit-Remaining: 0`,
  `RateLimit-Reset: <window-seconds>`).
- `_burst_rate_headers(...)` — success-path triple
  built from the live bucket state (no `Retry-After`).
- `_daily_quota_429(...)` — daily-quota 429 with
  `Retry-After: <until-utc-midnight>` + the same triple.

Each one hand-rolled the same dict construction. Every
helper had to re-decide:

- Header casing (`RateLimit-Limit` vs `ratelimit-limit`).
- `int()` coercion of the float seconds returned by
  `time.time()` arithmetic.
- Whether `RateLimit-Remaining` could go negative.
- Whether `Retry-After` belongs on the success path
  (it doesn't — that's a 429-only signal).

Per Huntova engineering review on shared-helper
interface guarantees: when three helpers redundantly
build the same wire-format contract, the next refactor
updates one branch and forgets another. BRAIN-126 (a495)
extracted the same shared-contract pattern for wizard
conflict responses — `_wizard_conflict_response` is the
single source of truth for the four conflict callsites.
This is the third-order extraction for rate-limit
headers.

a529 introduces `_rate_headers_for(*, limit, remaining,
reset_seconds, retry_after=None)` as the canonical
builder. All four parameters are keyword-only so a
future positional swap can't introduce a silent
contract drift. The helper:

- Always emits the IETF triple.
- Adds `Retry-After` iff `retry_after` is passed
  (success path passes `None`).
- Coerces every numeric input through `int()` so float
  seconds from `time.time()` arithmetic land safely.
- Clamps `remaining` to `>= 0` so a buggy caller never
  emits a negative budget (clients tend to interpret
  that via wraparound or just bail out — both bad).

The three legacy helpers now build their headers via
`_rate_headers_for(...)` instead of inline dict
literals. Behavioral parity: the existing 45 BRAIN-112
+ BRAIN-113 regression tests (Retry-After present,
RateLimit-Remaining = 0 on 429, full triple on the
success path, etc.) all pass unchanged.

12 new tests in `tests/test_rate_limit_helper_consolidation.py`:

- The shared builder exists and takes keyword-only args.
- Returns the IETF triple with no `Retry-After` when
  none is passed; includes `Retry-After` when one is.
- Clamps negative `remaining` to 0.
- Coerces float inputs to integer-stringy headers.
- Source-level: each of the three legacy helpers
  contains a `_rate_headers_for(` call so a future edit
  splitting one helper into a hand-rolled branch + a
  delegated branch fails the test.
- Source-level: `_attach_burst_rate_headers` continues
  to delegate to `_burst_rate_headers` (which now
  routes through the shared builder), inheriting the
  consolidation transitively.
- Defensive: unknown bucket → empty-dict no-op
  (preserved from a482).

Test count: 810 / 810 passing (+12 new tests in
`tests/test_rate_limit_helper_consolidation.py`).

### Files

- `server.py:582-636` — new `_rate_headers_for` builder; `_rate_limit_429`, `_burst_rate_headers`, `_daily_quota_429` refactored to delegate to it.
- `tests/test_rate_limit_helper_consolidation.py`: new — 12 regression tests pinning the consolidation.
