...
Idiomatic Python implementation of the launchd plist + systemd user-
unit patterns standard for long-running developer tools on macOS / Linux.
...
def _macos_uninstall() -> tuple[bool, str]:
    ...
    # Use -w so the unload is persisted to launchd's database. Without
    # it, the daemon would resurrect on next macOS reboot because the
    # OverrideDisabled flag never gets written.
...
def _systemd_escape(value: str) -> str:
    """Escape a value for inclusion in a systemd Environment= directive.

    systemd parses Environment="K=V" with shell-style rules: backslash
    escapes the next char, double-quote ends the value, and dollar
    triggers variable expansion. We escape all three so any keychain
    blob ships intact.
    """
...
def _build_systemd_unit(port: int, environment: dict[str, str] | None = None) -> str:
    ...
    if environment:
        for k, v in environment.items():
            if not v or "\n" in v or "\r" in v:
                continue
            env_lines.append(f'Environment="{k}={_systemd_escape(v)}"')
    env_block = ("\n".join(env_lines) + "\n") if env_lines else ""
    log_dir = _log_dir()
    return (
        "[Unit]\n"
