...
DEFAULT_SETTINGS = {
    ...
    # a240 — surfaces backed by the new chat-first UI's Settings panels:
    # hunt defaults that the chat system prompt + agent runner read by
    # name (default_max_leads, default_countries, default_tone) and the
    # preferred-provider chain that tells `providers.get_provider()`
    # which engine to pin for every AI call. preferred_temperature
    # threads through chat dispatcher + score validator.
