...
| Hook            | Fired                                          | Receives           |
|-----------------|------------------------------------------------|--------------------|
| `pre_search`    | Before SearXNG query batch is sent             | `(ctx, query_list)` |
| `post_search`   | After raw results return, before scoring       | `(ctx, results)`   |
| `pre_score`     | Before AI scoring of a single page             | `(ctx, page_text)` |
| `post_score`    | After AI scoring, before qualification         | `(ctx, score_dict)` |
| `post_qualify`  | After deep-qualify pass on a high-fit lead     | `(ctx, lead)`      |
| `post_save`     | After lead is written to local DB              | `(ctx, lead)`      |
| `pre_draft`     | Before email/LinkedIn draft is generated       | `(ctx, lead)`      |
| `post_draft`    | After draft is generated                       | `(ctx, drafts)`    |
...
Five reference plugins ship in the wheel and load on every hunt.
