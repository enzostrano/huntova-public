...
def _build_chain(bin_path: str, max_send: int) -> str:
    """Single shell command chaining the three daily commands.
    Each runs sequentially; failures don't stop the chain, so a flaky
    inbox check doesn't skip pulse."""
    return (f"{bin_path} sequence run --max {max_send}; "
