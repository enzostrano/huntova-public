...
def _key_for(provider_name: str, settings: dict[str, Any]) -> str | None:
    ...
    # a289 fix: strip + non-empty check. Was `os.environ.get(env_var)
    # or None` — empty-string env var (`export HV_OPENAI_KEY=""` from
    # a `.env` file with a commented-out key) would silently fall
    # through to the next provider in `_DEFAULT_ORDER`, with the user
    # getting a different model than expected and zero feedback.
...
class _OpenAICompatibleProvider:
    ...
    def chat(
        ...
        # `resp.usage.completion_tokens`). Most providers we route
        # through this class (OpenAI, Gemini OpenAI-compat, Groq,
        # OpenRouter, Together, DeepSeek, Mistral, Perplexity,
        # Ollama, LM Studio) populate usage on the response. Local
        # ones occasionally don't; fall through to None then.
        try:
            _u = getattr(resp, "usage", None)
            if _u:
                self._last_usage = {
                    "tokens_in":  int(getattr(_u, "prompt_tokens", 0) or 0),
                    "tokens_out": int(getattr(_u, "completion_tokens", 0) or 0),
...
class AnthropicProvider:
    ...
    def chat(
        ...
        # a240: capture real token usage so the chat UI can render
        # accurate "in / out / model" stats instead of a char/4
        # approximation. Anthropic's Message has a top-level `usage`
        # block: {input_tokens, output_tokens, cache_*}. Stash on the
        # provider instance for the caller to read after this returns.
        try:
            _u = getattr(resp, "usage", None)
            self._last_usage = {
                "tokens_in":  int(getattr(_u, "input_tokens", 0) or 0),
                "tokens_out": int(getattr(_u, "output_tokens", 0) or 0),
                "model": model_id,
                "approx": False,
            }
        except Exception:
...
def push_provider_override(slug: str | None) -> None:
    ...
    """
    # a289 fix: distinguish "clear the override" (None / empty) from
    # "set to an invalid slug". The previous version silently cleared
    # the override on any invalid input, which clobbered legitimate
    # earlier overrides set higher up the stack and gave the caller
    # zero feedback. Now: empty/None clears; unknown slug logs a
    # warning and is treated as a no-op (preserves the prior value).
    if slug is None or (isinstance(slug, str) and not slug.strip()):
        _provider_override.set("")
        try:
        ...
        return
    s = slug.strip().lower()
    if s not in _DEFAULT_ORDER:
        # Don't clobber a prior valid override — log + bail.
        import sys as _sys
        print(f"[providers] WARN: ignoring unknown provider override: {s!r} "
              f"(known: {sorted(_DEFAULT_ORDER)})", file=_sys.stderr)
        return
    _provider_override.set(s)
