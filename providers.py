...
# Thread-local provider override. A subagent (or any caller wanting
# per-call routing without rewriting every AI call site) can set the
# override on its own thread and `get_provider()` will honor it for
# every call from that thread until cleared. This is how the chat's
# AI selector + the multi-agent spawn fan-out picks per-agent providers.
import threading as _threading
_thread_provider = _threading.local()


def push_provider_override(slug: str | None) -> None:
    """Pin this thread's get_provider() to a specific slug, or clear
    when slug is None/empty. Called by agent_runner.spawn_subagent() so
    each daemon thread can target a different AI."""
    s = (slug or "").strip().lower()
    if s and s in _DEFAULT_ORDER:
        _thread_provider.slug = s
    else:
        try:
            del _thread_provider.slug
...
def get_provider(user_settings: dict[str, Any] | None = None) -> Provider:
    ...
    """
    settings = _resolve_settings(user_settings)
    # Thread-local override wins over settings — this is what makes the
    # multi-agent spawn fan-out actually run on different providers
    # without anyone rewriting their settings on disk.
    override = getattr(_thread_provider, "slug", None)
...
def _build(name: str, key: str, settings: dict[str, Any] | None = None) -> Provider:
    ...
    if name in _BASE_URL:
        return _GenericOpenAICompatProvider(
            ...
            base_url=(os.environ.get(f"HV_{name.upper()}_BASE_URL")
                      or settings.get(f"HV_{name.upper()}_BASE_URL")
                      or _BASE_URL.get(name)),
            default_model=(os.environ.get(f"HV_{name.upper()}_MODEL")
                           or settings.get(f"HV_{name.upper()}_MODEL")
                           or _DEFAULT_MODEL.get(name)
                           or "local-model"),
        )
    if name == "custom":
        # Custom endpoint: requires HV_CUSTOM_BASE_URL set in env / settings
        base_url = (os.environ.get("HV_CUSTOM_BASE_URL")
                    or settings.get("HV_CUSTOM_BASE_URL"))
        if not base_url:
            raise RuntimeError(
        ...
        return _GenericOpenAICompatProvider(
            ...
            base_url=base_url,
            default_model=(os.environ.get("HV_CUSTOM_MODEL")
                           or settings.get("HV_CUSTOM_MODEL")
                           or "custom-model"),
        )
