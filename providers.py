...
# Resolution priority — Gemini stays default for benchmark consistency,
# then local-AI options for cost-conscious users, then cloud premium.
_DEFAULT_ORDER = (
    "gemini", "anthropic", "openai", "ollama", "lmstudio", "llamafile",
...
class GeminiProvider(_OpenAICompatibleProvider):
    name = "gemini"
    _default_model = os.environ.get("HV_GEMINI_MODEL", "gemini-2.5-flash")
...
class OpenAIProvider(_OpenAICompatibleProvider):
    name = "openai"
    _default_model = os.environ.get("HV_OPENAI_MODEL", "gpt-4o-mini")
    ...
    def chat(
        ...
        for m in messages:
            if (m.get("role") or "").lower() == "system":
                ...
                kept.append(m)
        model_id = model or os.environ.get("HV_ANTHROPIC_MODEL", "claude-sonnet-4-5-20250929")
        kwargs: dict[str, Any] = {
            "model": model_id,
            "messages": kept,
            "max_tokens": max_tokens,
...
def get_provider(user_settings: dict[str, Any] | None = None) -> Provider:
    ...
    callers should catch this and direct the user to `huntova init` /
    setting an env var.
    """
    settings = _resolve_settings(user_settings)
    ...
    raise RuntimeError(
        "No LLM API key configured. Set HV_GEMINI_KEY (or "
        "HV_ANTHROPIC_KEY / HV_OPENAI_KEY) in your environment, run "
        "`huntova init` to onboard, or add `HV_GEMINI_KEY = \"...\"` "
        "to ~/.config/huntova/config.toml."
    )


def _build(name: str, key: str) -> Provider:
    if name == "gemini":
    ...
    if name in _BASE_URL:
        return _GenericOpenAICompatProvider(
            ...
            base_url=os.environ.get(f"HV_{name.upper()}_BASE_URL") or _BASE_URL.get(name),
            default_model=os.environ.get(f"HV_{name.upper()}_MODEL") or _DEFAULT_MODEL.get(name) or "local-model",
        )
    if name == "custom":
        # Custom endpoint: requires HV_CUSTOM_BASE_URL set in env / settings
        base_url = os.environ.get("HV_CUSTOM_BASE_URL") or settings.get("HV_CUSTOM_BASE_URL")
        if not base_url:
            raise RuntimeError(
        ...
        return _GenericOpenAICompatProvider(
            ...
            base_url=base_url,
            default_model=os.environ.get("HV_CUSTOM_MODEL") or "custom-model",
        )
