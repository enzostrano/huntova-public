...
Specialised for Huntova's lead-gen pipeline shape.
"""
...
# Strip ANSI codes when stdout is piped (`huntova pulse | grep ...` /
# redirected to a file) so downstream consumers see clean text.
...
async def _compute(user_id: int, since: timedelta) -> dict:
    ...
    for l in leads:
        ...
        if not d or d < cutoff:
            continue
        # Default unclassified replies (pre-a120 leads, or replies
        # marked via the dashboard before the IMAP classifier shipped)
        # to "unknown" — never silently inflate "interested".
        klass = (l.get("_reply_class") or "unknown").lower()
        by_class[klass] = by_class.get(klass, 0) + 1

    # Emails sent by sequence step.
    # a292 fix: a262 added optional 4-step cadence (cli_sequence.py
    # `follow_up_3_*` settings). The previous bound to {1,2,3} dropped
    # step-4 sends from the count, so installs that wired up a 4th
    # touch saw their "total_sent" under-report by however many step-4
    # emails fired in the window. Now: 4-key dict.
    sent_by_step = {1: 0, 2: 0, 3: 0, 4: 0}
    for l in leads:
        st = int(l.get("_seq_step") or 0)
        d = _parse_iso(l.get("_seq_last_at") or l.get("_sent_at"))
        if st > 0 and d and d >= cutoff:
            if st in sent_by_step:
                sent_by_step[st] += 1
...
def _format_human(p: dict) -> str:
    ...
    if any(sent.values()):
        ...
        # a292 fix: only show step 4 if anyone actually has it wired
        # — keeps the output uncluttered for installs running the
        # default 3-step cadence.
        if sent.get(4, 0):
            lines.append(f"    Step 4 (extra)  {sent.get(4, 0)}")
    ...
    if bc:
        ...
        for klass in ("interested", "not_now", "not_interested",
            ...
            # Pad the plain string FIRST, then colour. f"{coloured:28s}"
            # counts the ANSI escape bytes as characters and the column
            # alignment breaks when the helper is in TTY mode.
            lines.append(f"    {color(klass.ljust(28))} {n}")
        if p["total_sent"]:
