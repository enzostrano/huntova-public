...
async def _compute(user_id: int, since: timedelta) -> dict:
    ...
    for l in leads:
        ...
        if st > 0 and d and d >= cutoff:
            # Stability fix (audit wave 28): the previous version did
            # `sent_by_step[st] = sent_by_step.get(st, 0) + 1` which
            # silently created new keys for any `_seq_step` outside
            # {1, 2, 3} (a future 4th-touch cadence, schema drift, or
            # imported data). The human formatter only prints steps
            # 1/2/3, so the "total" line then disagreed with the
            # visible Step1+Step2+Step3 sum. Bound the count to the
            # advertised three-step cadence.
