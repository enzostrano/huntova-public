...
async def _dispatch_webhook_event(event: dict, event_type: str, event_id: str) -> dict:
    ...
    if event_type == "customer.subscription.updated":
        ...
        # a409 fix (BRAIN-48): record_webhook for ALL paths, not just
        # the side-effect-applied one. Pre-fix: user-not-found, no-tier-
        # change, and unknown-tier paths returned 200 without recording.
        # Sibling of BRAIN-46/47.
        first_writer_sub = await db.record_webhook(
            event_id, event_type, 0, "subscription.updated")
        if not first_writer_sub:
            return {"ok": True, "message": "Already processed (race-caught)"}
        # Locate user
        user = None
        if sub_meta.get("user_id"):
        ...
        # a409: record_webhook already ran above (top of branch); no
        # second claim needed. The original code re-claimed here but
        # the unique constraint on event_id makes it a no-op.
    ...
    # a410 fix (BRAIN-49): record-and-acknowledge unknown event types.
    # Pre-fix: any event_type not handled above (e.g. subscription.created,
    # payment_intent.succeeded, payment_method.attached, plus future
    # Stripe events) returned `ok: True, ignored: ...` without recording.
    # If Stripe later retries for any reason (unrelated event re-fire,
    # future schema change), the dispatcher would re-evaluate. Closes the
    # last idempotency gap by claiming every event_id we've seen.
    try:
        await db.record_webhook(event_id, event_type, 0, f"ignored:{event_type}")
    except Exception as _rec_err:
        print(f"[STRIPE webhook] ignored-record failed for {event_id} ({event_type}): {_rec_err}")
