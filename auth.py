...
async def login(email: str, password: str) -> tuple[dict, str]:
    ...
    # a289 fix: refuse login for unverified emails when verification
    # was sent. Soft check: only refuses if the user has a
    # `verification_token` still pending OR `email_verified` is the
    # explicit integer 0 (not None — not-yet-set means legacy account
    # we let through). Cloud installs that require verification add
    # the column with default 0; local installs leave verification
    # off via `auth_enabled=false` so this code never runs.
    _verified = user.get("email_verified")
    if _verified is not None and int(_verified) == 0:
