...
class PluginRegistry:
    ...
    def __init__(self) -> None:
        ...
        # a278: lock around `_load_errors`. The agent thread can append
        # via `run()`'s per-plugin exception handler at line 263 while a
        # FastAPI request thread reads the list via `errors()` (used by
        # `huntova plugins doctor` and the dashboard plugins panel).
        # Python list operations are atomic for single calls but a list
        # comprehension or copy during concurrent append can yield
        # inconsistent reads. Cheap insurance.
        import threading as _threading
        self._load_errors_lock = _threading.Lock()
    ...
    def discover(self) -> list[str]:
        ...
        try:
            ...
            self._record_load_error("(bundled)", f"{type(e).__name__}: {e}")
    ...
    def run(self, hook: str, ctx: HookContext, *args: Any) -> Any:
        ...
        for _prio, fn, plugin in chain:
            try:
                ...
                # a278: this append fires from the AGENT thread (run()
                # is called from the hunt loop) while `errors()` reads
                # from the API request thread — use the lock-protected
                # helper to avoid teared reads.
                self._record_load_error(plugin_name, f"{hook}: {type(e).__name__}: {e}")
        return carry
    ...
    def errors(self) -> list[tuple[str, str]]:
        # a278: lock the read so a concurrent agent-thread `_load_errors.append`
        # can't tear the snapshot.
        with self._load_errors_lock:
            return list(self._load_errors)

    def _record_load_error(self, name: str, message: str) -> None:
        """a278: lock-protected append helper. All append sites should
        funnel through this so a single audit point covers them all."""
        with self._load_errors_lock:
            self._load_errors.append((name, message))
    ...
    def _load_entry_points(self) -> list[str]:
        ...
        try:
            ...
            self._record_load_error("(entry_points)", str(e))
            return loaded
        for ep in eps:
            try:
                cls = ep.load()
                ...
                self._record_load_error(ep.name, f"{type(e).__name__}: {e}")
        return loaded
                ...
                for attr in dir(mod):
                    ...
                    if isinstance(obj, type):
                        ...
                        try:
                            ...
                            self._record_load_error(py_file.name, f"instantiate {attr}: {e}")
                            continue
                    else:
                    ...
                    try:
                        ...
                        self._record_load_error(py_file.name,
                            f"register {getattr(inst,'name',attr)}: {type(_re).__name__}: {_re}")
            except Exception as e:
                self._record_load_error(py_file.name, f"{type(e).__name__}: {e}")
        return loaded
