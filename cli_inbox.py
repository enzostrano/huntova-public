...
def _is_autoreply(subject: str, headers: list[tuple[str, str]]) -> bool:
    ...
    # bounce / DSN messages — never treat as a real reply. Only treat
    # the empty/null shape as a bounce-signal when the header is
    # actually present; missing headers happen on legitimate replies
    # too and shouldn't get auto-flagged.
    if "return-path" in h:
