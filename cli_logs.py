...
daemon stdout/stderr files. Wired in cli.py via `register(sub)`.
...
def register(sub) -> None:
    ...
    # a291 fix: bare `huntova logs` raised AttributeError because
    # logs_cmd was None and the dispatcher dereferenced --since /
    # --follow / --level / --json that hadn't been added at the
    # parent level. Defaults so the dispatcher can fall through to
    # the default "tail" path safely.
    p.set_defaults(func=cmd_logs, logs_cmd="tail",
                   since="", follow=False, level=None, json=False,
                   limit=200, lines=200)
