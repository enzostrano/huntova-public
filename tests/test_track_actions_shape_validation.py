"""Regression test for API-1 (a442 absorption): /api/track-actions
must validate that `actions` items are dicts before calling `.get()`.
...
the server emitted 500 in logs on every navigation, polluting
metrics and making real bugs harder to find.
