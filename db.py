...
def _merge_settings_sync(user_id: int, mutator) -> dict:
    ...
    try:
        ...
        # — which was why /api/wizard/save-progress was 500'ing as soon
        # as it migrated to merge_settings. The SELECT FOR UPDATE above
        # already goes through _xlate; this branch was missed.
        cur.execute(_xlate(
            "INSERT INTO user_settings (user_id, data) VALUES (%s, %s) "
            "ON CONFLICT (user_id) DO UPDATE SET data = %s"),
            [user_id, data_json, data_json])
