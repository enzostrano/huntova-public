...
-- save_lead_feedback uses ON CONFLICT (user_id, lead_id) for upsert.
-- The cloud init path also creates this via post-CREATE migration;
-- declaring it here means SQLite installs (which skip migrations)
-- still get it. Idempotent: IF NOT EXISTS.
CREATE UNIQUE INDEX IF NOT EXISTS idx_lead_feedback_user_lead ON lead_feedback(user_id, lead_id);
    ...
    try:
        ...
        if mode == "grant":
            ...
            cur.execute(
                ...
                "WHERE id = %s "
                "RETURNING (SELECT credits_remaining FROM old) AS old_balance, %s::int AS new_balance",
...
def _merge_lead_sync(user_id: int, lead_id: str, mutator) -> dict | None:
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            "SELECT data FROM leads WHERE user_id = %s AND lead_id = %s FOR UPDATE",
            [user_id, lead_id])
        row = cur.fetchone()
        if not row:
            conn.rollback()
...
async def upsert_lead(user_id: int, lead_id: str, data: dict) -> bool:
    ...
    row = await _afetchone(
        ...
        "ON CONFLICT (user_id, lead_id) DO UPDATE SET data=%s, fit_score=%s, org_name=%s, country=%s, email_status=%s "
        "RETURNING (xmax = 0) AS was_inserted",
        [user_id, lead_id, data_json, fit_score, org_name, country, email_status, now,
         data_json, fit_score, org_name, country, email_status])
...
async def save_leads_bulk(user_id: int, leads: list):
    ...
    for lead in leads:
        ...
        stmts.append((
            ...
            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s) "
            "ON CONFLICT (user_id, lead_id) DO UPDATE SET data=%s, fit_score=%s, org_name=%s, country=%s, email_status=%s",
            [user_id, lid, data_json, fs, on, co, es, ca, data_json, fs, on, co, es]))
    if stmts:
        await _aexec_pipeline(stmts)
...
def _merge_settings_sync(user_id: int, mutator) -> dict:
    ...
    try:
        ...
        cur.execute("SELECT data FROM user_settings WHERE user_id = %s FOR UPDATE", [user_id])
        row = cur.fetchone()
        try:
...
async def get_lead_feedback_recent(user_id: int, signal: str, limit: int = 10) -> list:
    rows = await _afetchall(
        ...
        "WHERE lf.user_id = %s AND lf.signal = %s ORDER BY lf.created_at DESC LIMIT %s",
