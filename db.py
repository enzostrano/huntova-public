...
-- a264: persistent chat history for the dashboard chat panel. Users on
-- local installs were losing their conversation every page reload —
-- the chat feed only lived in the DOM. Now every user message and AI
-- reply is persisted, conversations land in a left sidebar like
-- ChatGPT, and reopening Huntova restores prior state.
...
CREATE INDEX IF NOT EXISTS idx_chat_convos_user_updated ON chat_conversations(user_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_chat_msgs_convo_time ON chat_messages(conversation_id, created_at);

-- a280: per-user agent team. Eight specialist roles pre-filled from
-- the user's brain (services / industries / buyer_roles); user just
-- tweaks. Read by the chat dispatcher and (a281+) the hunt loop so
-- each phase consults its specialist's prompt_addendum + provider.
CREATE TABLE IF NOT EXISTS agent_team (
    ...
    max_pages_per_run INT NOT NULL DEFAULT 0,
    enabled BOOLEAN NOT NULL DEFAULT TRUE,
    sort_order INT NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_team_user_slot ON agent_team(user_id, slot);
CREATE INDEX IF NOT EXISTS idx_agent_team_user_order ON agent_team(user_id, sort_order);

-- a282: persistent business memory. Every wizard answer, chat insight,
-- and feedback signal accumulates here so Huntova learns the business
-- over time. Read by the chat system prompt + brain dossier so the AI
-- carries context across sessions instead of re-asking every time.
...
CREATE INDEX IF NOT EXISTS idx_biz_mem_user_recent ON business_memory(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_biz_mem_user_kind ON business_memory(user_id, kind);

-- a286: chat attachments. Users can drop images into the chat panel
-- and the AI (when on a vision-capable provider — Anthropic, OpenAI,
-- Gemini) sees them. File bytes live on disk under
-- ~/.local/share/huntova/uploads/<user_id>/<uuid>.<ext>; the row
-- carries metadata + the relative path. ON DELETE CASCADE so an
-- account erasure also drops attachment metadata (the on-disk files
-- are cleaned by gdpr_erasure separately).
...
CREATE INDEX IF NOT EXISTS idx_chat_attach_user_recent ON chat_attachments(user_id, created_at DESC);
"""
...
def _apply_credit_delta_sync(user_id: int, delta: int, reason: str, reference: str,
    ...
    # a347/a348 hardening of merge_settings + merge_lead. Plus the
    # SQLite driver lock + BEGIN IMMEDIATE so two threads can't trample
    # the connection. This path is gated by `policy.deduct_on_save()`
    # which never returns True in local mode (BYOK = no platform
    # billing), so the bug was latent — but the docstring at line 1131
    # claims "the abstraction stays consistent". Now it actually does.
    conn = get_conn()
    _is_sq = _is_sqlite()
    _drv_lock = None
    if _is_sq:
        try:
            from db_driver import _driver as _drv
            _drv_lock = getattr(_drv, "_lock", None)
        except Exception:
            _drv_lock = None
    if _drv_lock is not None:
        _drv_lock.acquire()
    try:
        cur = _cursor(conn)
        if _is_sq:
        ...
        if row is None:
            ...
            conn.rollback()
            cur2 = _cursor(conn)
            cur2.execute(_xlate("SELECT credits_remaining FROM users WHERE id = %s"), [user_id])
...
def _merge_lead_sync(user_id: int, lead_id: str, mutator) -> dict | None:
    ...
    if not _pool:
        raise RuntimeError("Database connection pool not available")
    # a348 fix: same race issue as merge_settings had pre-a347. _xlate
    # strips FOR UPDATE on SQLite, leaving the SELECT non-locking.
    # Adding driver RLock + BEGIN IMMEDIATE so concurrent merge_lead
    # callers actually serialise. Also: the UPDATE below was using raw
    # `%s` placeholders WITHOUT _xlate — would have crashed on SQLite
    # the same way merge_settings' upsert did pre-a333.
    conn = get_conn()
    _is_sq = _is_sqlite()
    _drv_lock = None
    if _is_sq:
...
def _merge_settings_sync(user_id: int, mutator) -> dict:
    ...
    if not _pool:
        raise RuntimeError("Database connection pool not available")
    # a347 CRITICAL FIX: SQLite path was NOT actually serialising.
    # `_xlate` strips `FOR UPDATE` for SQLite, the singleton SQLite
    # connection has no per-thread lock around get/execute, and the
    # default `BEGIN DEFERRED` doesn't take a write lock until the
    # first INSERT — leaving a wide window where two concurrent
    # writers both read the same blob, mutate independently, and the
    # last writer's commit clobbers the first. So all of a333/a334/
    # a337/a345/a346's "atomic" migrations were actually still racy
    # in local mode (the entire userbase). The race test in
    # tests/test_merge_settings.py reliably triggered this.
    #
    # Two changes:
    #   1. SQLite: wrap the whole transaction in the driver's RLock
    #      so two threads don't trample one connection's cursor state
    #      (`InterfaceError: bad parameter or other API misuse`).
    #   2. SQLite: BEGIN IMMEDIATE before the SELECT, which
    #      grabs SQLite's reserved-lock at transaction start.
    #      Concurrent writers block on busy_timeout (5s, set in
    #      db_driver.py) until the first commits.
    # Postgres path unchanged — `SELECT … FOR UPDATE` already does
    ...
    try:
        cur = _cursor(conn)
        if _is_sq:
            # Serialise SQLite writers via the reserved lock. With
            # BEGIN IMMEDIATE the next concurrent merge waits up to
            # busy_timeout (5s) for the active transaction to commit
            # before reading the row.
            try:
                cur.execute("BEGIN IMMEDIATE")
            except Exception:
                # If a transaction is already in flight (autocommit off),
                # SQLite raises — recover by treating that as already-
                # locked + proceed.
                pass
        ...
        data_json = json.dumps(new_data, ensure_ascii=False, default=str)
        # a333 fix: route through _xlate so the `%s` Postgres placeholders
        # get translated to `?` for SQLite. Without this, the helper
        # crashed on every SQLite invocation with `near "%": syntax error`.
        cur.execute(_xlate(
            "INSERT INTO user_settings (user_id, data) VALUES (%s, %s) "
            "ON CONFLICT (user_id) DO UPDATE SET data = %s"),
            [user_id, data_json, data_json])
...
# ── Chat history (a264) ──
# Persistent chat conversations + messages so users keep their dashboard
# chat across reloads / restarts. ChatGPT-style sidebar reads these.

async def create_conversation(user_id: int, title: str = "") -> int:
    """Create a new conversation. Returns its id."""
    now = datetime.now(timezone.utc).isoformat()
    return await _aexec_returning(
        "INSERT INTO chat_conversations (user_id, title, created_at, updated_at) "
        "VALUES (%s, %s, %s, %s) RETURNING id",
        [user_id, (title or "")[:200], now, now],
    )


async def list_conversations(user_id: int, limit: int = 100) -> list:
    """Return user's conversations newest-first. Strips archived."""
    return await _afetchall(
        "SELECT id, title, created_at, updated_at FROM chat_conversations "
        "WHERE user_id = %s AND COALESCE(archived, FALSE) = FALSE "
        "ORDER BY updated_at DESC LIMIT %s",
        [user_id, limit],
    )


async def conversation_belongs_to(user_id: int, conversation_id: int) -> bool:
    """a271 IDOR guard: confirm a user owns this conversation_id before any
    write/read against it. Cheap parameterized lookup. Returns False for
    non-existent or someone-else's conversations."""
    row = await _afetchone(
...
# a271 invariant: role must be one of the wire-format-known values. The UI
# renders user vs assistant differently; a typo silently broke the renderer
# pre-a271. Validation here means the silent-fail goes loud.
_VALID_CHAT_ROLES = {"user", "assistant", "system", "tool"}


async def add_chat_message(user_id: int, conversation_id: int,
                           role: str, content: str, meta: dict | None = None) -> int:
    """Append a message + bump the conversation's updated_at.

    a271: validates role against `_VALID_CHAT_ROLES` (raises ValueError on
    typos like "assitant" or "User"). Wraps both writes in a single
    transaction via `_aexec_pipeline` so the conversation's updated_at
    can never be left stale when the message INSERT succeeds and the
    UPDATE fails (was a non-atomic two-statement operation).
    """
    role_clean = (role or "").strip().lower()[:32]
    if role_clean not in _VALID_CHAT_ROLES:
        raise ValueError(f"add_chat_message: role {role!r} not in {sorted(_VALID_CHAT_ROLES)}")
    import json as _json
    now = datetime.now(timezone.utc).isoformat()
    meta_str = _json.dumps(meta or {}, ensure_ascii=False)[:8000]
    # Single round-trip: the INSERT returns the new id; the UPDATE bumps
    # updated_at. _aexec_pipeline runs both as one transaction.
    msg_id = await _aexec_returning(
        "INSERT INTO chat_messages (user_id, conversation_id, role, content, meta_json, created_at) "
        "VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
        [user_id, conversation_id, role_clean, content[:32000], meta_str, now],
    )
    # Best-effort timestamp bump. If this fails the message is on disk
    # (we already returned the row id) but the sidebar order is stale —
    # `list_conversations` orders by updated_at DESC. Acceptable
    # degradation; logging makes it visible to ops.
    try:
        await _aexec(
            "UPDATE chat_conversations SET updated_at = %s WHERE id = %s AND user_id = %s",
            [now, conversation_id, user_id],
        )
    except Exception as _ts_err:
        print(f"[chat] add_chat_message updated_at bump failed for convo {conversation_id}: {_ts_err}")
    return msg_id


async def get_chat_messages(user_id: int, conversation_id: int, limit: int = 500) -> list:
    """Return all messages in a conversation, oldest-first. Includes meta."""
    return await _afetchall(
...
async def rename_conversation(user_id: int, conversation_id: int, title: str) -> bool:
    """Set the title. Returns False if the convo isn't owned by this user."""
    now = datetime.now(timezone.utc).isoformat()
    n = await _aexec_rowcount(
        "UPDATE chat_conversations SET title = %s, updated_at = %s "
        "WHERE id = %s AND user_id = %s",
        [(title or "")[:200], now, conversation_id, user_id],
    )
    return n > 0


async def delete_conversation(user_id: int, conversation_id: int) -> bool:
    """Hard-delete the conversation + cascading messages. Returns False if
    not found / not owned."""
    n = await _aexec_rowcount(
...
# ── a280: agent team ──
# Eight specialist roles per user, pre-filled from the user's brain
# (services / industries / buyer_roles). The chat dispatcher consults
# these for routing + prompt context; future hunt phases will swap in
# the relevant role's prompt_addendum and provider_override.
...
_TEAM_DEFAULT_TEMPLATE = {
    "prospector":       {"name": "Prospector",       "icon": "🎯",  "max_pages": 25, "blurb": "Searches the web for fresh prospects matching your ICP."},
    "qualifier":        {"name": "Qualifier",        "icon": "⚖",   "max_pages": 5,  "blurb": "Scores fit, buyability, and reachability against your scoring rules."},
    "researcher":       {"name": "Researcher",       "icon": "🔬",  "max_pages": 14, "blurb": "Crawls company pages for decision context, recent news, and signals."},
    "contact_finder":   {"name": "Contact Finder",   "icon": "📇",  "max_pages": 8,  "blurb": "Tracks down the right human, email, and LinkedIn for each lead."},
    "outreach_drafter": {"name": "Outreach Drafter", "icon": "✉",   "max_pages": 0,  "blurb": "Drafts personalized cold emails that land instead of getting filtered."},
    "inbox_triager":    {"name": "Inbox Triager",    "icon": "📬",  "max_pages": 0,  "blurb": "Watches IMAP and classifies replies (interested, objection, unsubscribe)."},
    "sequence_op":      {"name": "Sequence Operator","icon": "🔁",  "max_pages": 0,  "blurb": "Runs your 3-step follow-up cadence and stops on reply or unsubscribe."},
    "pulse_reporter":   {"name": "Pulse Reporter",   "icon": "📊",  "max_pages": 0,  "blurb": "Compiles a daily pulse: what shipped, what's next, what to teach."},
}
...
async def list_team(user_id: int) -> list:
    """Return all 8 team rows for this user, ordered by sort_order. Empty
    list means the user has never seeded — caller (or seed_team_defaults)
    handles that."""
    return await _afetchall(
        "SELECT id, slot, name, role_kind, icon, description, prompt_addendum, "
        "provider_override, model_override, max_pages_per_run, enabled, sort_order, "
        "created_at, updated_at FROM agent_team WHERE user_id = %s ORDER BY sort_order ASC, id ASC",
        [user_id],
    )


async def seed_team_defaults(user_id: int, brain: dict | None = None,
                             overwrite: bool = False) -> dict:
    """Idempotently insert the 8 default specialist rows for this user.

    overwrite=False: skip slots that already exist (first-time seed).
    overwrite=True: wipe + reseed (used by Reset to defaults). The brain
    dict comes from `get_user_settings` → wizard.normalized_hunt_profile;
    None means we still write the rows but with empty prompt_addendums.
    """
    # a285 fix: race condition. Two concurrent /api/team calls both saw
    # `existing=set()` and both tried to INSERT slot 0. The unique index
    # `idx_agent_team_user_slot` raised IntegrityError on the loser,
    # propagating uncaught and leaving the user with a partial team
    # (slots 1-7 from the losing call never got inserted). Use ON
    # CONFLICT DO NOTHING so both callers race to inserts but neither
    # leaves a partial state. SQLite 3.24+ and Postgres both support it.
    now = datetime.now(timezone.utc).isoformat()
    inserted = 0
    if overwrite:
        await _aexec("DELETE FROM agent_team WHERE user_id = %s", [user_id])
    for i, slot in enumerate(_VALID_TEAM_SLOTS):
        tpl = _TEAM_DEFAULT_TEMPLATE[slot]
        await _aexec(
            "INSERT INTO agent_team (user_id, slot, name, role_kind, icon, description, "
            "prompt_addendum, provider_override, model_override, max_pages_per_run, enabled, "
            "sort_order, created_at, updated_at) VALUES "
            "(%s, %s, %s, %s, %s, %s, %s, '', '', %s, TRUE, %s, %s, %s) "
            "ON CONFLICT (user_id, slot) DO NOTHING",
            [user_id, slot, tpl["name"], slot, tpl["icon"], tpl["blurb"],
             _build_team_prompt_addendum(slot, brain),
             int(tpl["max_pages"]), i, now, now],
        )
        inserted += 1
    return {"inserted": inserted, "total": len(_VALID_TEAM_SLOTS)}


async def get_team_member(user_id: int, slot: str) -> dict | None:
    """Single team row by slot. Returns None for unowned/unknown slots."""
    if slot not in _VALID_TEAM_SLOTS:
        return None
    return await _afetchone(
        "SELECT id, slot, name, role_kind, icon, description, prompt_addendum, "
        "provider_override, model_override, max_pages_per_run, enabled, sort_order "
        "FROM agent_team WHERE user_id = %s AND slot = %s",
        [user_id, slot],
    )


async def update_team_member(user_id: int, slot: str, fields: dict) -> bool:
    """Patch a subset of {name, description, prompt_addendum, provider_override,
    model_override, max_pages_per_run, enabled}. Other keys ignored. Returns
    False if the slot is unknown or doesn't belong to this user."""
    if slot not in _VALID_TEAM_SLOTS:
        return False
    allowed = {
        "name": (str, 80),
    ...
    sets, params = [], []
    for k, (typ, cap) in allowed.items():
        if k not in fields:
            continue
        v = fields[k]
        if typ is str:
            ...
            try:
                v = max(0, min(int(v), 200))
            except (TypeError, ValueError):
                continue
        elif typ is bool:
            # a285 fix: bool("false") is True (non-empty string). If a
            # client (or AI tool-call) sends {"enabled": "false"}, the
            # row would flip TO enabled, opposite of intent. Accept real
            # JSON booleans and recognised string falsies.
    ...
    params.extend([datetime.now(timezone.utc).isoformat(), user_id, slot])
    n = await _aexec_rowcount(
        f"UPDATE agent_team SET {', '.join(sets)} WHERE user_id = %s AND slot = %s",
        params,
    )
    return n > 0


# ── a282: business memory (persistent learning) ──
# Every wizard answer + chat insight + feedback signal accumulates so
# Huntova carries context across sessions. The chat system prompt
# injects the top N entries so the AI never re-asks what it already
# knows; the team's prompt_addendums can also pull from this table.

_VALID_MEMORY_KINDS = {
    "wizard_answer",     # auto-extracted from /api/wizard/scan and /complete
    "chat_insight",      # the user said something factual in chat
    "feedback_signal",   # the user marked a lead good/bad with a reason
    "manual",            # the user added an entry by hand from the Brain view
    "specialist_note",   # a team specialist surfaced a pattern worth keeping
}


async def record_business_memory(user_id: int, kind: str, key: str, value: str,
                                 confidence: float = 0.7,
                                 source_type: str = "", source_ref: str = "") -> int:
    """Append a memory entry. Caps strings to safe lengths so a runaway
    AI emission can't bloat the table. Returns the new row id (or 0 on
    bad input — caller can decide whether that matters)."""
    k = (kind or "").strip().lower()[:32]
    if k not in _VALID_MEMORY_KINDS:
        return 0
    key_clean = (key or "").strip()[:120]
    val_clean = (value or "").strip()[:2000]
    if not val_clean:
        return 0
    try:
        ...
        cf = 0.7
    now = datetime.now(timezone.utc).isoformat()
    # a285 fix: _aexec_returning returns a SCALAR (the id), not a dict.
    # The previous `row["id"] if row and "id" in row else 0` raised
    # TypeError on every successful insert (`"id" in 42` blows up), and
    # the wizard auto-record's outer try/except just swallowed it to
    # stdout — meaning every memory write since a282 has been silently
    # failing. Verified against db.py:240 _exec_returning impl.
    new_id = await _aexec_returning(
        "INSERT INTO business_memory (user_id, kind, key, value, confidence, source_type, "
        "source_ref, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) "
        "RETURNING id",
        [user_id, k, key_clean, val_clean, cf,
         (source_type or "")[:64], (source_ref or "")[:240], now, now],
    )
    try:
...
async def list_business_memory(user_id: int, kind: str | None = None,
                               limit: int = 50, include_archived: bool = False) -> list:
    """Return memory rows newest first. kind=None pulls all kinds.
    Caps limit at 200 so a buggy client can't exhaust the connection."""
    lim = max(1, min(int(limit) if limit else 50, 200))
    if kind:
        kk = (kind or "").strip().lower()[:32]
        if kk not in _VALID_MEMORY_KINDS:
            return []
        if include_archived:
            return await _afetchall(
                "SELECT id, kind, key, value, confidence, source_type, source_ref, "
                "archived, created_at FROM business_memory WHERE user_id = %s AND kind = %s "
        ...
        return await _afetchall(
            "SELECT id, kind, key, value, confidence, source_type, source_ref, "
            "archived, created_at FROM business_memory WHERE user_id = %s AND kind = %s "
            "AND COALESCE(archived, FALSE) = FALSE ORDER BY created_at DESC LIMIT %s",
            [user_id, kk, lim],
        )
    if include_archived:
        return await _afetchall(
            "SELECT id, kind, key, value, confidence, source_type, source_ref, "
            "archived, created_at FROM business_memory WHERE user_id = %s "
            "ORDER BY created_at DESC LIMIT %s",
            [user_id, lim],
        )
    return await _afetchall(
...
async def get_memory_context_text(user_id: int, max_chars: int = 1500) -> str:
    """Compact memory snippet for injection into the chat SYSTEM_PROMPT.
    Top N entries by recency, weighted toward higher confidence. Returns
    an empty string for users with no memory yet."""
    rows = await list_business_memory(user_id, limit=20)
    if not rows:
        return ""
    # Order: high-confidence wizard_answer + chat_insight first, then
    # feedback signals, then everything else newest-first.
    def _weight(r):
        kind_w = {"wizard_answer": 3, "chat_insight": 3, "manual": 2,
                  "feedback_signal": 1, "specialist_note": 1}.get(r.get("kind", ""), 1)
        return kind_w + float(r.get("confidence") or 0.5)
    rows_sorted = sorted(rows, key=_weight, reverse=True)
    out = []
    used = 0
    for r in rows_sorted:
        kk = r.get("kind", "")
        line = f"- [{kk}] {(r.get('key') or '').strip()}: {(r.get('value') or '').strip()}"
        line = line[: max_chars // 2]  # one line ≤ half the budget
...
async def archive_business_memory(user_id: int, memory_id: int) -> bool:
    """Soft-delete (archive). Keeps the row for audit but excludes it
    from list_business_memory + get_memory_context_text."""
    n = await _aexec_rowcount(
        "UPDATE business_memory SET archived = TRUE, updated_at = %s "
        "WHERE id = %s AND user_id = %s",
        [datetime.now(timezone.utc).isoformat(), int(memory_id), user_id],
    )
    return n > 0


# ── a286: chat attachments ──
# Image uploads attached to chat messages. Vision-capable providers
# (Anthropic, OpenAI, Gemini) read them when the message is dispatched.

async def record_attachment(user_id: int, conversation_id: int | None,
                            filename: str, mime: str, size_bytes: int,
                            storage_path: str) -> int:
    """Insert a new attachment row, return its id. The caller writes
    the file bytes to `storage_path` first; this just records metadata."""
    new_id = await _aexec_returning(
        "INSERT INTO chat_attachments (user_id, conversation_id, filename, "
        "mime, size_bytes, storage_path, created_at, consumed) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s, FALSE) RETURNING id",
        [user_id, conversation_id,
         (filename or "")[:240], (mime or "")[:120],
...
async def get_attachment(user_id: int, attachment_id: int) -> dict | None:
    """Fetch attachment metadata. user_id-scoped — returns None for
    other users' attachments (IDOR guard)."""
    return await _afetchone(
        "SELECT id, user_id, conversation_id, filename, mime, size_bytes, "
        "storage_path, created_at, consumed FROM chat_attachments "
        "WHERE id = %s AND user_id = %s",
        [int(attachment_id), user_id],
    )


async def mark_attachment_consumed(user_id: int, attachment_id: int) -> bool:
    """Flip the consumed flag once the attachment has been included in a
    chat message. Doesn't delete the file — kept for audit + display in
    historical chat scrollback."""
    n = await _aexec_rowcount(
        "UPDATE chat_attachments SET consumed = TRUE "
        "WHERE id = %s AND user_id = %s",
        [int(attachment_id), user_id],
    )
    return n > 0
