...
def _send_email_sync(to: str, subject: str, html_body: str, plain_body: str = "",
    ...
    a302: optional `unsub_url` is the HTTPS one-click unsubscribe URL
    (RFC 8058). When provided, it's added to the `List-Unsubscribe`
    header alongside the mailto: form so Gmail/Yahoo bulk-sender
    policy (Feb 2024+) is satisfied. Without it, deliverability is
    materially worse for cold outreach.
    """
    s = _smtp_settings()
    ...
    # a414 fix (BRAIN-53): same CRLF rejection on the configured
    # `from_email` setting. Pre-fix `to` was scrubbed but the SMTP
    # `from_email` flowed unchecked into the From header, the
    ...
    if not _from_email_raw or "\r" in _from_email_raw or "\n" in _from_email_raw:
        raise ValueError(f"Invalid configured from_email: {_from_email_raw!r}")
    _from_name_parsed, _from_addr_parsed = parseaddr(_from_email_raw)
    # parseaddr returns ('','') for unparseable input; demand a real
    # addr and reject any embedded controls.
    if not _from_addr_parsed or "\r" in _from_addr_parsed or "\n" in _from_addr_parsed:
        raise ValueError(f"Invalid configured from_email: {_from_email_raw!r}")
    ...
    # a295: wire previously-silent `reply_to` Settings → Email field.
    # When set, replies go there instead of from_email — useful when a
    # team sends from `noreply@` but wants replies routed to a real
    # mailbox. Validated for header-injection same as To/Subject.
    _reply_to = (s.get("reply_to") or "").strip()
    if _reply_to:
        _rt_name, _rt_addr = parseaddr(_reply_to)
        if _rt_addr and "\r" not in _rt_addr and "\n" not in _rt_addr:
            msg["Reply-To"] = formataddr((_scrub_header(_rt_name, 200), _rt_addr))
    ...
    # a414 (BRAIN-53): _from_addr_parsed already vetted for CRLF above.
    _domain = _from_addr_parsed.split("@")[-1] or "huntova.local"
    ...
    # a302: include HTTPS one-click URL when caller passed it. Gmail
    # POSTs to the first non-mailto URI it finds when the user clicks
    # Unsubscribe in the inbox UI. Without an HTTPS URL, Gmail falls
    # back to opening compose with the mailto — most users abandon.
    # a414 (BRAIN-53): also CRLF-guard the unsub_url before interpolating.
    _unsub_safe = ""
    if unsub_url and "\r" not in unsub_url and "\n" not in unsub_url:
        _unsub_safe = unsub_url
    if _unsub_safe:
        msg["List-Unsubscribe"] = f"<{_unsub_safe}>, <mailto:{_from_addr_parsed}?subject=unsubscribe>"
    else:
        msg["List-Unsubscribe"] = f"<mailto:{_from_addr_parsed}?subject=unsubscribe>"
    ...
    # a414 (BRAIN-53): use the parsed/validated from-address, not the
    # raw setting (which a CRLF check above already rejects, but
    # belt-and-braces — sendmail's envelope must match the From header).
    try:
        if int(s["port"]) == 465:
            with smtplib.SMTP_SSL(s["host"], s["port"], context=_tls_context, timeout=15) as server:
                server.login(s["user"], s["password"])
                server.sendmail(_from_addr_parsed, to, msg.as_string())
        else:
