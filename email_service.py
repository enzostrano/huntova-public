...
def _scrub_header(value: str, max_len: int = 998) -> str:
    """a289 fix: strip CR/LF + control bytes from any string headed for
    an email header. Defends against header injection — AI-generated
    subjects (and worse, AI scrapings of hostile pages) used to land
    verbatim in `Subject:` / `To:` / `Reply-To:`. A subject containing
    `Hello\\r\\nBcc: attacker@x.com` would be folded into the SMTP DATA
    stream as a separate Bcc header and silently exfiltrate copies of
    every cold email to the attacker. Python's `email.message` library
    folds long lines but does NOT reject embedded CRLF. RFC 5322 caps
    a single header line at 998 octets — we cap there too. Drops C0
    control bytes (except tab) which can corrupt some MTAs. The result
    is safe to assign into `msg[header]` for any string-typed header."""
    if value is None:
        return ""
    s = str(value)
