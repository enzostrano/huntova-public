...
def _is_pending(lead: dict) -> bool:
    ...
    AND has a contact email to send to.

    Stability fix (audit wave 28): the previous version returned True
    purely on fit_score >= 8, with no check for contact_email. Leads
    lacking an email address entered the queue and `huntova approve
    --top N` bulk-approved them. They then progressed to
    `outreach send`, where the lack of an email address either
    silently no-op'd the send or — depending on lead-state — burned
    a credit on a no-op. The qualifier already hard-rejects "no
    contact path" leads at agent-step 14, but a lead can be in the
    DB without contact_email if enrichment failed mid-run.
    """
    ...
    if status == "awaiting_approval":
        return has_email
    return _safe_int(lead.get("fit_score")) >= 8 and has_email
