...
def _read_landing_with_version() -> str:
    ...
    try:
        ...
        if v:
            html = html.replace("v0.1.0a184", f"v{v}")
            html = html.replace("0.1.0a184", v)
    except Exception:
        pass
    return html
...
async def api_wizard_save_progress(request: Request, user: dict = Depends(require_user)):
    ...
    # a322 fix: the brain wizard (templates/jarvis.html:_BRAIN_QUESTIONS)
    # sends ACTUAL field names — `company_name`, `business_description`,
    # `target_clients`, `services`, `buyer_roles`, `regions`,
    # `example_good_clients`, `exclusions`, `outreach_tone`,
    # `company_website` — but this endpoint was only mapping the legacy
    # names from the long-deprecated AI-interview wizard
    # (`business_name`, `what_you_do`, `ideal_customer`, etc.). Result:
    # progressive saves persisted to `_wizard_answers` but never
    # updated the top-level fields used by `/api/wizard/status` →
    # trained-summary card → "Brain trained on <stale name>". Users
    # reported "brain forgotten on every run" because they retrained
    # with new info, never finished `/api/wizard/complete`, and on
    # reload the summary card kept showing the previous run's data.
    # Map the real names through every save so the top-level fields
    # always reflect the latest in-progress answers.
    _DIRECT_FIELDS = (
