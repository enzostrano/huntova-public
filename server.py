...
def _get_client_ip(request: Request) -> str:
    ...
    forwarded = forwarded_header.split(",")[0].strip() if forwarded_header else ""
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://accounts.google.com https://apis.google.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://api.fontshare.com; font-src 'self' https://fonts.gstatic.com https://cdn.fontshare.com; img-src 'self' data: https:; connect-src 'self' https://accounts.google.com; frame-src https://accounts.google.com"
...
async def startup():
    ...
    try:
        ...
        if all_users and not any(u.get("role") in ("admin", "superadmin") for u in all_users):
            first = min(all_users, key=lambda u: u["id"])
...
async def verify_email(token: str = ""):
    ...
    if token_uid:
        ...
        if (user.get("email") or "").lower().strip() != (email or "").lower().strip():
...
async def ops_page(request: Request):
    ...
    if user.get("role") not in ("admin", "superadmin"):
        # Fallback bootstrap for ADMIN_EMAILS
...
a{{color:#36dfc4;text-decoration:none}}a:hover{{color:#7aecd8}}
...
a{{color:#36dfc4;text-decoration:none;font-weight:600}}</style></head>
...
.logo{{text-align:center;margin-bottom:28px}}.logo span{{font-family:'Geist Mono','JetBrains Mono',monospace;font-size:18px;font-weight:700}}.logo .a{{color:#eceef6}}.logo .b{{color:#36dfc4}}
...
input:focus{{border-color:#36dfc4;box-shadow:0 0 0 3px rgba(54,223,196,.06)}}
button{{width:100%;padding:14px;border-radius:6px;background:#36dfc4;color:#070a0e;font:700 14px/1 'Satoshi',-apple-system,system-ui,sans-serif;border:none;cursor:pointer;margin-top:4px}}
button:hover{{box-shadow:0 4px 24px rgba(54,223,196,.2)}}
...
async def download_page():
    """OpenClaw-style download / install landing for the CLI."""
    return _read_template("download.html")
...
_COMPARE_DATA = {
    "clay": {
        "name": "Clay",
        "tagline": "vs. Clay",
        "lede": "Clay is a powerful enrichment workflow builder. Huntova returns evidenced leads — fewer rows, more proof.",
        "rows": [
            ("Pricing", "$149+/mo per seat", "Open source · BYOK"),
            ("Data freshness", "Ages with cached datasets", "Live web on every run"),
            ("Source proof", "Mixed providers; no per-cell trail", "Quoted evidence + URL per lead"),
            ("Where it runs", "Hosted SaaS — your data on their servers", "Local CLI — SQLite on your machine"),
            ("Reproducibility", "Workflows re-run, but no diff/recipe history", "huntova recipe save / run with diff vs prior run"),
            ("AI choice", "Locked to their stack", "BYOK Gemini, Anthropic, or OpenAI"),
            ("Distribution", "Login flow per teammate", "Share /h/<slug> URLs · recipients fork the hunt locally"),
            ("Extensibility", "Workflow nodes only", "Python plugin protocol with 8 lifecycle hooks"),
            ("Lock-in", "Export = CSV from their UI", "Your data is already in your local SQLite"),
        ],
        "verdict": "If you need to enrich a list of contacts you already have → use Clay. If you need to FIND new prospects with proof → use Huntova.",
    },
    "apollo": {
        "name": "Apollo.io",
        "tagline": "vs. Apollo.io",
        "lede": "Apollo sells access to a 270M-contact static database. Huntova builds a fresh prospect list per query, with proof.",
        "rows": [
            ("Pricing", "$59+/seat/mo · paywalled exports", "Open source · BYOK"),
            ("Data origin", "Static database aged ~weeks-months", "Live web crawl per query"),
            ("Per-lead evidence", "Generic firmographics", "Quoted evidence + 'why now' thesis"),
            ("Reachability proof", "Email/Phone tier (verified/risky/unknown) — opaque", "5-tier waterfall + proof trail per contact path"),
            ("Where it runs", "Cloud SaaS", "Local CLI"),
            ("Hunt vs filter", "Filter their database", "Generate ICP-specific search queries → live web"),
            ("Privacy", "Your queries logged on their servers", "Queries never leave your machine"),
            ("Cost per lead", "Quota-based", "Whatever your AI provider charges (≈ $0.001-0.01/lead)"),
            ("Lock-in", "Their database, their format", "SQLite + JSON · grep your own data"),
        ],
        "verdict": "If you need 5,000 cached contacts in one filter → Apollo. If you need 50 fresh, evidenced prospects ready to outreach → Huntova.",
    },
    "hunter": {
        "name": "Hunter.io",
        "tagline": "vs. Hunter.io",
        "lede": "Hunter is a domain-to-email lookup. Huntova does the upstream prospecting that decides which domains to look up in the first place.",
        "rows": [
            ("Pricing", "$49+/mo · 500 searches", "Open source · BYOK"),
            ("Primary job", "Find emails for a known company", "Find COMPANIES that fit your ICP"),
            ("Output", "Email addresses", "Qualified leads with email + evidence + outreach draft"),
            ("Data origin", "Pattern detection across web", "Live web reading + AI scoring"),
            ("Reachability proof", "Verification badge", "5-tier reachability waterfall"),
            ("Use together?", "Hunter is the email-finder", "Huntova can call Hunter via plugin (post_qualify)"),
        ],
        "verdict": "Use them together — Huntova hunts, Hunter verifies. A reference Hunter plugin will ship with the wheel soon.",
    },
}


@app.get("/compare/{competitor}", response_class=HTMLResponse)
async def compare_page(competitor: str):
    """Side-by-side comparison page — Tab 0 launch checklist priority 5.

    Catches search traffic for "Clay alternative" / "Apollo alternative"
    / "Hunter.io alternative" and pitches Huntova on its actual
    differentiators. /compare/<name> for known competitors; 404 for
    unknown.
    """
    data = _COMPARE_DATA.get((competitor or "").lower())
    if not data:
        return HTMLResponse(_compare_not_found(competitor), status_code=404)
    return HTMLResponse(_render_compare_page(data))


def _compare_not_found(competitor: str) -> str:
    from html import escape as _esc
    safe = _esc(competitor or "")
    avail = " · ".join(f"<a href='/compare/{k}'>/{k}</a>" for k in _COMPARE_DATA)
    return _render_share_shell(
        title=f"No comparison page for {safe}",
        body=(
            "<main class='wrap'><h1>No comparison yet for "
            f"{safe!r}.</h1>"
            f"<p>Available comparisons: {avail}.</p>"
            f"<p><a class='cta' href='{_share_cta_url()}'>Try Huntova →</a></p></main>"
        ),
        og_description="Huntova — open-source local-first lead generation.",
    )


def _render_compare_page(data: dict) -> str:
    from html import escape as _esc
    name = data["name"]
    rows_html = "".join(
        f"<tr><td class='cmp-axis'>{_esc(axis)}</td>"
        f"<td class='cmp-them'>{_esc(them)}</td>"
        f"<td class='cmp-us'>{_esc(us)}</td></tr>"
        for axis, them, us in data["rows"]
    )
    body = (
        "<main class='wrap'>"
        "<p class='back-link' style='margin:0 0 18px;font-size:13px'>"
        "<a href='/landing' style='color:var(--mute);text-decoration:none;border-bottom:1px dotted var(--mute)'>← Back to huntova.com</a>"
        "</p>"
        "<header class='hero'>"
        f"<p class='kicker'>{_esc(data['tagline'])}</p>"
        f"<h1>Huntova {_esc(data['tagline'])}</h1>"
        f"<p class='dateline'>{_esc(data['lede'])}</p>"
        "</header>"
        "<section style='margin-top:24px'>"
        "<table class='cmp-table'>"
        f"<thead><tr><th></th><th>{_esc(name)}</th><th>Huntova</th></tr></thead>"
        f"<tbody>{rows_html}</tbody>"
        "</table>"
        "</section>"
        "<aside class='cta-block' style='margin-top:32px'>"
        f"<p><strong>Bottom line.</strong> {_esc(data['verdict'])}</p>"
        f"<a class='cta' href='{_share_cta_url()}'>Install Huntova →</a> "
        f"&nbsp;<a class='cta' style='background:transparent;border:1px solid var(--line);color:var(--ink)' href='/demo'>See a sample Proof Pack →</a>"
        "</aside></main>"
    )
    return _render_share_shell(
        title=f"Huntova vs {name} — Huntova",
        body=body,
        og_description=f"How Huntova compares to {name}. Live web hunting, evidence-first, BYOK, local-first, open source.",
    )
...
async def try_page():
    """Live ICP scratchpad — Tab 0 round-70 launch-day spike."""
    return _read_template("try.html")
...
async def api_metric(request: Request):
    ...
    if not isinstance(raw_props, dict):
        raw_props = {}
    # Strip anything that looks like PII before persisting
    props_clean = {}
...
async def demo_page():
    ...
    sample = {
        ...
        "meta": {
            ...
            # Triggers the "Preview-generated sample" banner on the share
            # page renderer — same surface as /try-minted shares so the
            # boundary between sample data and real CLI output is always
            # visible.
            "demo": True,
        },
...
"""
    css = """
    body{margin:0;background:#08090c;color:#eef0f4;font-family:-apple-system,BlinkMacSystemFont,Inter,'Segoe UI',sans-serif;line-height:1.55}
    ...
    if do_test:
        try:
            ...
            try:
                ...
                test_passed = False
                test_message = f"{type(e).__name__}: {str(e)[:120]}"
        except RuntimeError as e:
            test_passed = False
            test_message = f"provider init: {str(e)[:120]}"
    return {
        "ok": True,
        "provider": provider,
        "backend": backend,
...
async def api_bulk_update(request: Request, user: dict = Depends(require_user)):
    ...
    for l in leads:
        ...
        if old == st:
            continue
        l["email_status"] = st
        l["email_status_date"] = now
        h = l.get("status_history", [])
        h.append({"status": st, "date": now})
        l["status_history"] = h
        try:
            await db.upsert_lead(user["id"], l["lead_id"], l)
            updated += 1
        except Exception as _err:
            print(f"[BULK] upsert failed for user {user['id']} lead {l.get('lead_id')}: {_err}")
...
_SHARE_LEAD_FIELDS = (
    ...
    "contact_role", "contact_linkedin", "org_linkedin",
...
def _render_share_shell(title: str, body: str, og_description: str = "",
    ...
    *{box-sizing:border-box}
    html,body{margin:0;padding:0;background:var(--bg);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Helvetica,Arial,sans-serif;line-height:1.5}
    a{color:var(--ink)}
...
async def api_save_settings(request: Request, user: dict = Depends(require_user)):
    ...
    for k in ("booking_url", "from_name", "from_email", "phone", "website", "ai_provider"):
        if k in body:
            s[k] = body[k]
        ...
        try:
            ...
            def _fetch(url):
                try:
                    ...
                    if _is_private(url):
                        return ""
                    r = _rq.get(url, headers={"User-Agent": USER_AGENT}, timeout=12, verify=False)
...
def _analyse_site_ai_sync(site_text: str, url: str, mode: str = "full") -> str:
    ...
    prompt = f"""You are a senior B2B sales intelligence analyst. A company wants to use AI to find their ideal clients.
Analyse this website thoroughly and extract everything a lead generation AI would need.
...
5. What industries do their clients come from? (Look at case studies, testimonials, client logos)
...
{{
  ...
  "summary": "4-6 sentence description covering: what they do, who they serve, how they deliver, and what makes them different. Write this as if you're briefing a sales analyst who needs to understand the business in 30 seconds.",
  "business_type": "one sentence: specific type of business and core offering",
  "services": ["list of 3-8 SPECIFIC services they offer — not vague categories"],
  "how_it_works": "2-3 sentences: the actual delivery process from sale to completion",
  "industries_served": ["specific industries visible from clients, case studies, or industry language"],
  "target_clients": "detailed: who buys this — org type, size, role, situation that triggers purchase",
  "differentiators": ["3-5 specific competitive advantages — not generic 'quality' but real moats"],
  "price_tier": "budget|midrange|premium|enterprise",
  "regions": ["geographic areas from addresses, client locations, language, currency"],
  ...
  "buyer_roles": ["job titles that make the buying decision"],
  "buying_triggers": ["specific situations that make someone seek this service RIGHT NOW"],
  "confidence": 0
}}"""
    ...
    for m in models:
        try:
            resp = _byok_chat(**_ai_json_kwargs(
                model=m, messages=[
                    {"role": "system", "content": "You are a senior B2B intelligence analyst. Think deeply about the business before responding. Output ONLY valid JSON — no markdown, no commentary."},
                    {"role": "user", "content": prompt}],
...
async def api_wizard_scan(request: Request, user: dict = Depends(require_user)):
    ...
    print(f"[SCAN] wizard/scan: {len(site_text)} chars via {result['method']} → {final_url[:80]}")
    try:
        ...
        if analysis and isinstance(analysis, dict):
            ...
            return analysis
        return JSONResponse({"error": "AI analysis failed", "fallback": True})
    except Exception as e:
...
async def api_wizard_complete(request: Request, user: dict = Depends(require_user)):
    ...
    # Persist ALL incoming profile fields directly (new 6-step wizard fields)
    for k, v in profile.items():
        if v is not None and k not in ("_interview_complete",):
            w[k] = v
    ...
    try:
        ...
        try:
            def _fallback():
                ...
                return (resp.choices[0].message.content or "").strip()
            reply = await asyncio.to_thread(_fallback)
            reply = _re.sub(r'<think>.*?</think>', '', reply, flags=_re.DOTALL).strip()
            return {"ok": True, "reply": reply}
        except Exception as e2:
            print(f"[WIZARD ASSIST] Flash also failed: {e2}")
            return {"ok": False, "error": "AI temporarily unavailable. Try again."}
...
async def api_lead_feedback(request: Request, user: dict = Depends(require_user)):
    ...
    should_refine = total > 0 and total % 10 == 0
    if should_refine:
            ...
            try:
                ...
                if profile:
                    ...
                    await db.save_learning_profile(
                        ...
                        profile.get("instruction_summary", ""),
                        total,
                        ver
                    )
...
async def api_runtime():
    ...
    """
    from runtime import CAPABILITIES
    return {"ok": True, "runtime": CAPABILITIES.to_dict()}
