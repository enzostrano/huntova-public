...
def _merge_wizard_answers(prev, incoming) -> dict:
    ...
    silently wiped all saved answers. Now: empty / non-dict
    incoming is a no-op, full incoming is merged with collisions
    going to incoming (user revising an answer)."""
    prev_d = prev if isinstance(prev, dict) else {}
    if not isinstance(incoming, dict) or not incoming:
        return dict(prev_d)
    return {**prev_d, **incoming}
...
async def api_wizard_complete(request: Request, user: dict = Depends(require_user)):
    ...
    brain = _build_hunt_brain(_w_snap)
    ...
    dossier = _generate_training_dossier(_w_snap, brain)
