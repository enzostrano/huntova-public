...
async def api_wizard_complete(request: Request, user: dict = Depends(require_user)):
    ...
    # bus drop / reconnect; durable state survives. The pending
    # marker was set inside the merge mutator above so this
    # closure only writes the terminal state.
    async def _gen_dna():
...
async def api_wizard_save_progress(request: Request, user: dict = Depends(require_user)):
    ...
    if _stale["value"]:
        # a442 (BRAIN-81): epoch mismatch is structurally different
        # from revision mismatch — wizard was RESET, not just
        # edited. Distinct response so client can reload + clear
        # local state rather than retry with stale answers.
        if _stale.get("kind") == "wizard_reset":
            return JSONResponse(
                {
                    "ok": False,
                    "error": "Wizard was reset elsewhere. Reload to start fresh.",
                    "error_kind": "wizard_reset",
                    "current_epoch": _stale["current_epoch"],
