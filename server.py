...
async def landing_page():
    # a258: substitute the live version into the landing template so
    # the hero badge / install snippet / latest-release pill stay in
    # sync with cli.py's VERSION instead of drifting (was stuck on
    # v0.1.0a184 — 74 versions stale).
    html = _read_template("landing.html")
    try:
        v = _huntova_version() or ""
        if v:
            # The template uses "v0.1.0a184" in three spots and the
            # bare "0.1.0a184" form in one. Replace both.
...
async def api_chat(request: Request, user: dict = Depends(require_user)):
    ...
    if _check_ai_rate(user["id"]):
        return JSONResponse({"action": "answer",
                            ...
                            status_code=429)
    # a279: source-aware action gating. cli_remote (Telegram bridge) and
    # any future low-trust transport sets `body["source"] = "telegram"`
    # to opt into the safer subset. The dashboard itself doesn't set
    # source (defaults to "web") and retains full action access. Any
    # value other than "web" is treated as low-trust.
    _chat_source = (body.get("source") or "web").strip().lower()[:32]
    _LOW_TRUST_SOURCES = {"telegram", "remote", "sms"}
    _is_low_trust = _chat_source in _LOW_TRUST_SOURCES
...
async def api_save_settings(request: Request, user: dict = Depends(require_user)):
    ...
    for k in ("booking_url", "from_name", "from_email", "phone", "website",
              ...
              "calendly_workspace", "hubspot_api_token",
    ...
    for sm_url in sitemap_urls:
        ...
        try:
            ...
            if r.status_code != 200 or len(r.text) < 50:
                continue
            # Pick out <loc>...</loc> entries — works for both sitemap.xml
            # AND sitemap-index (which we'll then expand).
            locs = re.findall(r"<loc>\s*(https?://[^<\s]+)\s*</loc>", r.text, re.IGNORECASE)
            child_indexes = []
            for loc in locs:
            ...
            for child in child_indexes[:10]:
                ...
                try:
                    ...
                    if cr.status_code == 200:
                        clocs = re.findall(r"<loc>\s*(https?://[^<\s]+)\s*</loc>",
