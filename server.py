...
_RATE_BUCKETS = {
    ...
    "memory": (60, 20),                 # a533 (BRAIN-145): /api/memory write+archive
}
...
@app.post("/api/memory")
async def api_memory_record(request: Request, response: Response,
                            user: dict = Depends(require_user)):
    ...
    a533 (BRAIN-145): adjacent-AI-surface
    hardening parity. Migrated from the legacy
    default `ai` bucket + bare 429 to the
    BRAIN-91/112/113/117 contract — dedicated
    `memory` bucket so manual writes don't fight
    chat / wizard for budget, `_rate_limit_429`
    for IETF Retry-After + RateLimit-* headers,
    `_attach_burst_rate_headers` on success, and
    `_enforce_body_byte_cap` before JSON parse so
    multi-MB payloads short-circuit at the front
    door (memory rows feed the chat SYSTEM_PROMPT,
    so unbounded bodies are a poisoning vector)."""
    if _check_ai_rate(user["id"], bucket="memory"):
        return _rate_limit_429(user["id"], "memory",
                               "Too many memory writes. Wait a moment.")
    _attach_burst_rate_headers(response, user["id"], "memory")
    _body_bytes, _too_large = await _enforce_body_byte_cap(request, _WIZARD_BODY_BYTES_MAX)
    if _too_large is not None:
        return _too_large
    try:
...
async def api_memory_archive(memory_id: int, response: Response,
                             user: dict = Depends(require_user)):
    ...
    a533 (BRAIN-145): adjacent-AI-surface
    hardening parity. Shares the `memory` bucket
    with `api_memory_record` so a rogue client
    can't burn through writes via the archive
    side-channel. No body byte-cap (DELETE has
    no body), but the rate-limit + RateLimit-*
    header contract still applies."""
    if _check_ai_rate(user["id"], bucket="memory"):
        return _rate_limit_429(user["id"], "memory",
                               "Too many memory mutations. Wait a moment.")
    _attach_burst_rate_headers(response, user["id"], "memory")
