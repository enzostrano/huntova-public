...
async def api_chat(request: Request, user: dict = Depends(require_user)):
    ...
    _approx_in = max(1, _prompt_chars // 4)
    _approx_out = max(0, _out_chars // 4)
    _chat_meta = {
        "engine": (getattr(prov, "name", "") or "").lower() or "auto",
        "model": (getattr(prov, "model", "") or "") or "",
        "latency_ms": int(_dt * 1000),
        "tokens_in": _approx_in,
        "tokens_out": _approx_out,
        "tps": round(_approx_out / _dt, 1),
        "approx": True,  # flag to UI: these are char/4 approximations
    }
