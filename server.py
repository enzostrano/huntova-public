...
# a529 (BRAIN-146): single source of truth for the IETF
# RateLimit-Limit / RateLimit-Remaining / RateLimit-Reset
# triple (+ optional Retry-After) shared by every rate-
# limit-related helper below. Per BRAIN-126 (a495)
# extracted the same shared-contract pattern for wizard
# conflict responses; this is the third-order extraction
# for rate-limit headers. Without one builder, the three
# downstream helpers (`_rate_limit_429`,
# `_daily_quota_429`, `_burst_rate_headers`) drift over
# time — one tightens the int() coercion, another
# changes header casing, a third forgets `Retry-After`
# capping. Centralising the construction pins all three
# to one canonical contract.
def _rate_headers_for(
    *,
    limit: int,
    remaining: int,
    reset_seconds: int,
    ...
    """Build the IETF RateLimit-* header triple (+ an
    optional `Retry-After`) as a `dict[str, str]` ready to
    be passed to `JSONResponse(headers=...)` or
    `.update()`'d onto an existing response.

    Arguments are keyword-only so callsites stay readable
    at every level. All inputs are coerced via `int()` so
    floats from `time.time()` arithmetic land safely as
    seconds. `remaining` is clamped to `>= 0` so a buggy
    caller never emits a negative remaining (clients tend
    to interpret that as "huge budget left" via wraparound
    or just bail out — both bad).

    Header keys use the exact casing required by the IETF
    draft + RFC 6585: `Retry-After`, `RateLimit-Limit`,
    `RateLimit-Remaining`, `RateLimit-Reset`.
    """
    headers = {
        "RateLimit-Limit": str(int(limit)),
        "RateLimit-Remaining": str(max(0, int(remaining))),
        "RateLimit-Reset": str(int(reset_seconds)),
    }
...
def _rate_limit_429(user_id: int, bucket: str, message: str, error_kind: str | None = None):
    ...
    a529 (BRAIN-146): header construction routed through
    `_rate_headers_for` so this helper, `_daily_quota_429`,
    and `_burst_rate_headers` all share one builder.
    """
    window, max_calls = _RATE_BUCKETS.get(bucket) or (AI_RATE_WINDOW, AI_RATE_MAX)
    payload = {"ok": False, "error": message}
    if error_kind:
        payload["error_kind"] = error_kind
    headers = _rate_headers_for(
        limit=int(max_calls),
        remaining=0,
...
def _burst_rate_headers(user_id: int, bucket: str) -> dict:
    ...
    a529 (BRAIN-146): triple construction routed through
    `_rate_headers_for`; success path passes
    `retry_after=None` so only the limit/remaining/reset
    keys are emitted.
    """
    ...
    used = len(attempts)
    remaining = int(max_calls) - used
    return _rate_headers_for(
        limit=int(max_calls),
        remaining=remaining,
        reset_seconds=int(window),
    )
