...
async def api_chat(request: Request):
    """Web-chat dispatcher — same JSON-action shape as `huntova chat`
    CLI. Sends user message + system prompt to the configured provider,
    parses the JSON action, returns it for the frontend to dispatch.

    Bound to local mode for now (no auth). In cloud mode we'd require
    a session and rate-limit per user.
    """
    try:
        body = await request.json()
    except Exception:
    ...
    SYSTEM_PROMPT = (
        "You are Huntova's web-chat dispatcher. Reply with EXACTLY ONE "
        "JSON object — no markdown, no fences, no prose. Schema:\n"
        '  {"action":"start_hunt","countries":["Germany"],"max_leads":10,'
        '"timeout_minutes":15,"icp":"<short ICP>"}\n'
        '  {"action":"list_leads","filter":"country:Germany"}\n'
        '  {"action":"answer","text":"<helpful reply>"}\n'
        "Rules: countries is a list of full English country names "
        "(e.g. 'Germany', 'United Kingdom', 'United States'). Use "
        "action=answer for how-to / status / general questions."
    )
    ...
    if not isinstance(parsed, dict) or "action" not in parsed:
        return {"action": "answer", "text": "Reply missing action field."}
    return parsed
...
async def api_save_settings(request: Request, user: dict = Depends(require_user)):
    ...
    _ALLOWED_PLUGINS = {"csv-sink", "dedup-by-domain", "slack-ping", "recipe-adapter", "adaptation-rules"}
...
async def api_smtp_test(user: dict = Depends(require_user)):
    ...
    try:
        ...
        return JSONResponse({"ok": False, "error": "auth_failed",
                             "message": f"Auth rejected: {getattr(e,'smtp_error',b'').decode('utf-8','ignore')[:120]}"})
...
async def api_account_export(request: Request, user: dict = Depends(require_user)):
    ...
    fname = f"huntova_account_{datetime.now().strftime('%Y%m%d')}.json"
...
async def api_research(request: Request, user: dict = Depends(require_user)):
    ...
    if _check_ai_rate(user["id"]):
        return JSONResponse({"error": "Too many requests. Wait a moment."}, status_code=429)
    # Provider pre-flight (local mode) — fail fast with the actionable
    # "Open Settings → Providers" message instead of letting the AI call
    # collapse mid-research.
    try:
        from runtime import CAPABILITIES as _CAPS
        if _CAPS.mode == "local":
            from providers import list_available_providers
...
async def api_generate_agent_dna(request: Request, user: dict = Depends(require_user)):
    ...
    # Provider pre-flight (local mode): DNA generation is an AI call.
    # Reject early with a clear message instead of letting the call
    # fail mid-flight with an opaque provider error.
    try:
        from runtime import CAPABILITIES as _CAPS
        if _CAPS.mode == "local":
            from providers import list_available_providers
