...
async def api_chat(request: Request, user: dict = Depends(require_user)):
    ...
    SYSTEM_PROMPT = (
        "You are Huntova's web-chat brain — like OpenClaw's chat but for "
        "B2B lead-gen. Reply with EXACTLY ONE JSON object, no markdown, "
        "no fences, no prose around it.\n\n"
        f"LIVE STATE: {_state}\n\n"
        ...
        '    → use for how-to, status, general questions.\n\n'
        ...
        "user explicitly said 'yes / confirm / delete it'."
    )
    ...
    try:
        ...
        if act == "mint_share":
            ...
            ids = [l.get("lead_id") for l in leads if l.get("lead_id")]
            slug = await db.create_hunt_share(
                user_id=user["id"], run_id=None,
                lead_ids=ids, title=title or None,
            )
            url = f"{PUBLIC_URL.rstrip('/')}/h/{slug}"
            return {"action": "done",
                    "text": f"Minted: {url}",
                    "result": {"slug": slug, "url": url, "count": len(ids)}}
    except Exception as e:
        return {"action": "answer",
