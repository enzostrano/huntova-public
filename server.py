...
def _check_rate_limit(ip: str) -> bool:
    ...
    # The block below is the original per-IP login/signup limiter shape
    # — kept identical to its pre-a279 behaviour. The ops-mutator path
    # uses a separate, per-admin limiter at `_check_admin_mutator_rate`
    # below so a stolen admin session-cookie can't enumerate or mint
    # credits at full speed from any IP.
    pass  # explicit no-op so the diff stays scoped to the comment
    # Prune old entries
...
async def api_chat(request: Request, user: dict = Depends(require_user)):
    ...
    try:
        ...
        # a338: humanise common provider errors so the chat doesn't read
        # as "fake AI". Was: opaque "AI call failed: APIStatusError:
        # Error code: 402 - {'error': {'message': '…'}}" — user has no
        # idea what to do. Now: detect 401 (bad/missing key), 402
        # (out of credits / payment required), 429 (rate limit) and
        # surface a clear next-step message with the provider name.
        _err_str = str(e)
        _provider_name_disp = (provider_name or "AI").upper()
        _hint = ""
        if "401" in _err_str or "Unauthorized" in _err_str or "invalid_api_key" in _err_str.lower():
            _hint = (f"Your {_provider_name_disp} API key is invalid or missing. "
                     f"Check Settings → Engine → {_provider_name_disp} key.")
        elif "402" in _err_str or "credit" in _err_str.lower() or "insufficient" in _err_str.lower():
            _hint = (f"Your {_provider_name_disp} account is out of credits. "
                     f"Top up your provider account, then retry.")
        elif "429" in _err_str or "rate" in _err_str.lower() and "limit" in _err_str.lower():
            _hint = (f"{_provider_name_disp} is rate-limiting. Wait 30-60s and retry, "
                     f"or switch provider in the Engine dropdown.")
        elif "404" in _err_str and "model" in _err_str.lower():
            _hint = (f"The configured model isn't available on {_provider_name_disp}. "
                     f"Check Settings → Engine → preferred model.")
        else:
            _hint = (f"AI call to {_provider_name_disp} failed: {type(e).__name__}: "
                     f"{_err_str[:200]}")
        return await _persist_chat_reply({"action": "answer", "text": _hint})
...
"""
    ...
    def _do_research_safe():
        try:
            ...
            # a342: humanise the error so the user knows what to do
            # (same pattern as a338 chat + a341 scan). Was: opaque
            # "Research failed: APIStatusError: …" — user thinks it's
            # a Huntova bug.
            _u_str = str(_unexpected)[:200]
            _hint = ""
            if "401" in _u_str or "Unauthorized" in _u_str or "invalid_api_key" in _u_str.lower():
                _hint = " — your AI provider's API key is invalid; fix it in Settings → Engine."
            elif "402" in _u_str or "credit" in _u_str.lower() or "insufficient" in _u_str.lower():
                _hint = " — your AI provider account is out of credits. Top up and retry."
            elif "429" in _u_str or ("rate" in _u_str.lower() and "limit" in _u_str.lower()):
                _hint = " — your AI provider is rate-limiting; wait 60s and retry."
            elif "404" in _u_str and "model" in _u_str.lower():
                _hint = " — the configured model isn't available; check Settings → Engine → preferred model."
            err_msg = f"Deep research failed: {type(_unexpected).__name__}: {_u_str}{_hint}"
            try:
                loop = asyncio.new_event_loop()
                try:
...
async def api_wizard_scan(request: Request, user: dict = Depends(require_user)):
    ...
    try:
        ...
        # a341 fix: surface specific provider errors (401/402/429)
        # like the chat dispatcher does, plus mention the crawl
        # succeeded so the user doesn't think they need to retype the
        # URL or wait out a 200-page recrawl on retry.
        _err_str = str(e)[:240]
        _hint = ""
        if "401" in _err_str or "Unauthorized" in _err_str or "invalid_api_key" in _err_str.lower():
            _hint = " Your AI provider's API key is invalid — fix it in Settings → Engine and retry."
        elif "402" in _err_str or "credit" in _err_str.lower() or "insufficient" in _err_str.lower():
            _hint = " Your AI provider account is out of credits — top up and retry."
        elif "429" in _err_str or ("rate" in _err_str.lower() and "limit" in _err_str.lower()):
            _hint = " Your AI provider is rate-limiting — wait 60s and retry."
        elif "404" in _err_str and "model" in _err_str.lower():
            _hint = " The configured model isn't available on this provider — check Settings → Engine → preferred model."
        msg = (f"Crawled {pages_seen} page{'s' if pages_seen != 1 else ''} via {crawl_method}, "
               f"but the AI summarisation step failed.{_hint or ' Try again in a moment.'}")
        return JSONResponse({
            "error": msg,
            "fallback": True,
            "crawl_ok": True,
...
async def api_wizard_generate_phase5(request: Request, user: dict = Depends(require_user)):
    ...
    for key, lbl in field_labels:
        val = _stringify(answers.get(key))
        if val:
            summary.append(f"{lbl}: {val[:600]}")
    ...
    if isinstance(scan_data, dict):
        for key, lbl in [
            ...
            v = _stringify(scan_data.get(key))
            if v:
                scan_extras.append(f"{lbl}: {v[:400]}")
    ...
    try:
        ...
        if js:
            ...
            if isinstance(questions, list) and len(questions) >= 3:
                # Defensive shape-cleaning: ensure every item has the
                # fields the client expects, default missing prefill to "".
                cleaned = []
                for q in questions[:5]:
                    ...
                    cleaned.append({
                        ...
                        "type": str(q.get("type") or "text").strip(),
