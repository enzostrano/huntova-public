...
async def auth_login(request: Request):
    ...
    try:
        ...
        # Log the specific reason server-side for ops debugging, but
        # surface the same generic 401 to the client whether the email
        # is unknown or the password is wrong. Distinguishing the two
        # is a known account-enumeration vector.
        print(f"[auth] login rejected for {email!r}: {e}", file=sys.stderr)
...
# 404s in production until the flag is flipped for design partners.
...
@app.post("/api/chat")
async def api_chat(request: Request, response: Response, user: dict = Depends(require_user)):
    ...
    a523 (BRAIN-142): chat dispatcher hardening parity
    with the wizard surface — `chat` bucket in
    `_RATE_BUCKETS`, `_enforce_body_byte_cap` before
    parse, `_rate_limit_429` for IETF backoff headers
    on 429, `_attach_burst_rate_headers` for proactive
    throttling on success, Idempotency-Key replay so a
    lost-response retry doesn't re-spend tokens. Per
    Huntova engineering review on adjacent-AI-surface
    parity.
    """
    # Burst-rate gate first (cheap denial). Use the
    # dedicated `chat` bucket — sharing the default
    # `ai` bucket caused cross-callsite starvation.
    if _check_ai_rate(user["id"], bucket="chat"):
        return _rate_limit_429(user["id"], "chat", "Too many chat requests. Wait a moment.")
    _attach_burst_rate_headers(response, user["id"], "chat")
    # Idempotency-Key replay: a lost-response retry
    # with the same key returns the original response
    # body + status, no token re-spend.
    ...
    if _idem_cleaned:
        ...
        if _idem_hit is not None:
            print(f"[IDEMPOTENCY] user={user['id']} chat replay key={_idem_cleaned[:16]}...")
            return JSONResponse(
                _idem_hit["body"],
                status_code=int(_idem_hit["status"]),
            )
    # Byte-cap BEFORE parse so an oversize chat body
    # rejects 413 without paying parse cost.
    ...
    SYSTEM_PROMPT = (
        ...
        'the URL yourself.\n'
        '- {"action":"update_settings","patch":{"default_max_leads":25,"booking_url":"https://cal.com/me","from_name":"Your Name","theme":"dark","reduced_motion":false}}\n'
    ...
    try:
        ...
        # a523 (BRAIN-142): persist the dispatcher
        # response under the client-supplied
        # Idempotency-Key (if any) so a subsequent
        # retry with the same key replays the original
        # response body + 200 status, no token re-spend
        # on the same logical chat operation.
        if _idem_cleaned and isinstance(parsed, dict):
            await _idempotency_store(user["id"], _idem_cleaned, 200, parsed)
        return parsed
    finally:
...
# is here so the flag can be flipped to ship v1.1 in a single deploy
# whenever the timing is right. The flag stays off until then.
...
@app.post("/api/leads/{lead_id}/linkedin_pack")
async def api_lead_linkedin_pack(lead_id: str, user: dict = Depends(require_user)):
    """Generate a peer-to-peer LinkedIn outreach package for a single saved lead.

    Returns a copy-paste-ready package: decision-maker role, LinkedIn SEARCH url
    (never an invented profile slug), connection-request message in the prospect's
    likely working language, follow-up DM. BYOK — costs the user's own AI tokens.
    """
    require_feature(user, "research")
    if _check_ai_rate(user["id"], bucket="linkedin_pack"):
        return _rate_limit_429(user["id"], "linkedin_pack",
                               "Too many LinkedIn-pack requests. Wait a moment.")
    _np = _local_no_provider_response()
    if _np is not None:
        return _np

    lead = await db.get_lead(user["id"], lead_id)
    if not lead:
        return JSONResponse({"error": "not found"}, status_code=404)

    # Pull the user's hunt brain so the AI knows which buyer roles to target
    # and what the user's offer is (used only to inform tone — never pitched
    # in the connection request itself).
    from app import _generate_linkedin_pack as _gen_pack, _build_hunt_brain
    try:
        _settings = await db.get_settings(user["id"]) or {}
    except Exception:
        _settings = {}
    _wiz = _settings.get("wizard") if isinstance(_settings, dict) else {}
    try:
        _brain = _build_hunt_brain(_wiz) if isinstance(_wiz, dict) else {}
    except Exception:
        _brain = {}

    _page_text = ""
    for _k in ("page_text", "snippet", "why_fit", "production_gap"):
        _v = lead.get(_k) if isinstance(lead, dict) else None
        if isinstance(_v, str) and _v.strip():
            _page_text = (_page_text + "\n\n" + _v).strip() if _page_text else _v
    _page_text = _page_text[:6000]

    # Run the AI call off the event loop — _ai_call is sync.
    pack = await asyncio.get_event_loop().run_in_executor(
        None, _gen_pack, lead, _brain, _page_text,
    )

    # Best-effort persist on the lead via atomic merge (same pattern as
    # api_research at server.py:8340) so concurrent writes don't lose-update.
    if isinstance(pack, dict) and not pack.get("error"):
        try:
            def _pack_save_mutator(current):
                current = dict(current or {})
                current["linkedin_pack"] = pack
                if pack.get("linkedin_search_url"):
                    current["linkedin_search_url"] = pack["linkedin_search_url"]
                if pack.get("connection_request_message"):
                    current["connection_request"] = pack["connection_request_message"]
                if pack.get("follow_up_dm"):
                    current["linkedin_followup_dm"] = pack["follow_up_dm"]
                return current
            await db.merge_lead(user["id"], lead_id, _pack_save_mutator)
        except Exception:
            # Persistence failure is non-fatal — the user still gets the pack
            # back in the response and can copy from there.
            pass

    return JSONResponse({"ok": True, "lead_id": lead_id, "pack": pack})
...
async def api_wizard_save_progress(request: Request, response: Response, user: dict = Depends(require_user)):
    ...
    # so a network failure mid-response is much more
    # common here. Per Huntova engineering review on
    # client retry safety: retry semantics matter MORE
    # on the high-frequency write than on the terminal
    # one. Order: rate check → idempotency lookup →
    # byte cap → json parse → mutator → success-return
    # → store.
    _idem_key_raw = request.headers.get("idempotency-key") or ""
    _idem_cleaned = _idempotency_key_clean(_idem_key_raw)
    if _idem_cleaned:
    ...
    # under the client-supplied Idempotency-Key (if any)
    # so a subsequent retry with the same key replays
    # the same body + 200 status. Best-effort: the
    # helper logs + swallows internally.
    if _idem_cleaned:
        await _idempotency_store(user["id"], _idem_cleaned, 200, _success_body)
    return _success_body
...
async def api_update_release_notes(version: str = ""):
    ...
    try:
        ...
        # Log the actual filesystem / permission / encoding error for
        # ops, but don't echo str(exc) to clients — exception text from
        # pathlib / IO can include absolute server paths.
        print(f"[changelog] read failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        return {"ok": False, "error": "Unable to read changelog"}
