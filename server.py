...
async def api_chat(request: Request, user: dict = Depends(require_user)):
    ...
    """
    # CSRF defense for the exempt path. Browser-originated POSTs
    # carry an Origin header — reject any that isn't our own. Server-
    # side scripts (cli_remote, curl) don't set Origin, so they pass.
    ...
    try:
        ...
        if act == "mint_share":
            ...
            # Defensive: if every top-N lead lacks a lead_id (data drift,
            # AI selecting stale rows, partial migration), `create_hunt_share`
            # would happily mint an empty share. Bail with a friendly
            # message instead so the user knows what happened.
            if not ids:
                return {"action": "answer",
...
async def api_recipe_publish(request: Request):
    ...
    if _content_length and _content_length > 256 * 1024:
