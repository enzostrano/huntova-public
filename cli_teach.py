...
def _maybe_refine_dna(user_id: int, pre_total: int = -1) -> dict | None:
    """If total feedback just CROSSED a x10 mark this session, run DNA
    refinement. Returns the new DNA dict or None. Same code path as the
    dashboard's /api/lead-feedback POST handler in server.py.

    Stability fix (audit wave 27): the previous version checked
    `total % 10 == 0` post-save with no comparison against the pre-save
    total. `save_lead_feedback` is an UPSERT — re-rating already-rated
    leads leaves `total` unchanged. So a teach session that produced
    zero new signals while the total happened to sit at any multiple
    of 10 (10, 20, 30, …) re-fired DNA refinement on every session
    end — wasted AI call, no new data feeding it. Caller now passes
    `pre_total` (the count BEFORE the session's writes); we only fire
    when the decade actually advanced.
    """
    ...
    async def _run() -> dict | None:
        ...
        # When pre_total is provided (>= 0), require an actual
        # decade crossing — i.e. total // 10 strictly increased.
        # Old callers that pass the default sentinel still get
        # the legacy mod-10-only behavior, so this is non-breaking.
        if pre_total >= 0 and (pre_total // 10) >= (total // 10):
            return None
        try:
...
def _cmd_interactive(user_id: int, args: argparse.Namespace) -> int:
    ...
    if not leads:
        ...
        return 0
    # Snapshot pre-session feedback total so DNA refinement only fires
    # when this session actually crosses a decade boundary (a214).
    _pre_counts = _asyncio.run(_db.get_lead_feedback_count(user_id)) or {}
    _pre_total = (_pre_counts.get("good") or 0) + (_pre_counts.get("bad") or 0)
    ...
    if good + bad > 0:
        dna = _maybe_refine_dna(user_id, pre_total=_pre_total)
        if dna:
...
def _cmd_import(user_id: int, args: argparse.Namespace) -> int:
    import asyncio as _asyncio
    import db as _db
    # Snapshot pre-session total for the same reason as _cmd_interactive
    # — UPSERT save_lead_feedback means re-rated leads don't change
    # total, so without a pre/post comparison every teach-import call
    # at any multiple of 10 retriggers DNA refinement (a214).
    _pre_counts = _asyncio.run(_db.get_lead_feedback_count(user_id)) or {}
    _pre_total = (_pre_counts.get("good") or 0) + (_pre_counts.get("bad") or 0)
