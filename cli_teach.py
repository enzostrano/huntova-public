"""Huntova `teach` — guided good-fit / bad-fit flow boosting the
smart-loop without dashboard clicks. Verbs: `<interactive>` /
`--import <csv>` (cols org_name,verdict; fuzzy match) / `status`.
Same lead_feedback path as `server.py /api/lead-feedback`. Wired in
cli.py via register(sub). Fresh huntova-only feature."""
...
# colors (mirror cli_approve.py / cli_memory.py shape)
...
def _norm_org(s: str) -> str:
    ...
    Round-11 finding #3: NFKD-normalise so accented chars match their
    plain counterparts. "Café Société" ↔ "cafe societe" ratios at
    ~0.83 only when length is similar; longer accented names dropped
    below the 0.78 threshold and silently failed --import matching.
    """
...
def _fuzzy_find(needle: str, leads: list) -> dict | None:
    """0.78 = 'Acme Corp' ≈ 'Acme Corporation' without drifting into noise."""
    ...
    for lead in leads:
        ...
        r = 1.0 if cand == target else SequenceMatcher(None, target, cand).ratio()
        if r > best_ratio: best_ratio = r; best = lead
    return best if best_ratio >= 0.78 else None

# DNA refinement (mirrors server.py /api/lead-feedback path)
def _maybe_refine_dna(user_id: int) -> dict | None:
    """Run DNA refinement if total feedback just hit a x10 mark.
    Same code path as the dashboard's /api/lead-feedback POST handler."""
...
def _render_lead(lead: dict, idx: int, total: int) -> None:
    print(f"\n  {_dim(f'{idx}/{total}')}  {_bold(lead.get('org_name') or '(unknown)')}  "
          f"{_fit_chip(lead.get('fit_score'))}")
    for k, lbl in (("org_website", "site:    "), ("country", "country: "),
                   ("industry", "industry:"), ("why_fit", "why_fit: ")):
        v = (lead.get(k) or (lead.get("sector") if k == "industry" else "") or "").strip()
        if v: print(f"    {_cyan(lbl)} {v[:200]}")
...
def _cmd_interactive(user_id: int, args: argparse.Namespace) -> int:
    ...
    try: from tui import intro, outro, select, SelectOption
        ...
        print("[huntova] tui unavailable — install questionary or run a TTY.", file=sys.stderr)
        return 1
    ...
    good = bad = skipped = 0
    # Round-11 finding #4: Ctrl+C mid-teach used to skip the outro +
    # _maybe_refine_dna call, so a user who'd just rated 4 leads at
    # the x10 boundary lost the DNA refinement permanently. Now we
    # break out of the loop on KeyboardInterrupt and still run the
    # post-loop summary + refine.
    try:
        for i, lead in enumerate(sample, start=1):
            ...
            if choice == "skip" or not lid: skipped += 1; continue
            try:
        ...
        print()  # newline after the half-rendered prompt
        print(f"  {_dim('(stopped — keeping what you rated)')}")
    outro(f"Recorded {_green(str(good))} good · {_red(str(bad))} bad · "
          f"{_dim(str(skipped) + ' skipped')}")
    if good + bad > 0:
        dna = _maybe_refine_dna(user_id)
        if dna:
            print(f"  {_green('●')} Agent DNA refined (v{dna.get('version', 1)}, "
...
def _cmd_import(user_id: int, args: argparse.Namespace) -> int:
    ...
    if not path:
        print("[huntova] usage: huntova teach --import <csv>", file=sys.stderr); return 1
    try:
...
def _cmd_status(user_id: int, args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(_json.dumps({"good": g, "bad": b, "total": total,
                           "until_next_dna_refinement": until}, indent=2)); return 0
    print(f"\n  {_bold('Smart-loop feedback')}\n    good       {_green(str(g))}\n"
          f"    bad        {_red(str(b))}\n    total      {total}")
    tail = ("Run `huntova teach` to start teaching the agent." if total == 0
            else f"Next DNA refinement in {until} more signal(s) (every 10).")
    print(f"\n  {_dim(tail)}\n"); return 0
...
def register(sub) -> None:
    ...
    t = sub.add_parser("teach",
        ...
        description="Guided good-fit / bad-fit flow recording lead_feedback "
                    "rows + triggering DNA refinement every 10 signals — "
                    "same path the dashboard buttons use. Verbs: "
                    "`<interactive>` / `--import <csv>` / `status`.")
