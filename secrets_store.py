...
def _derive_key() -> bytes:
    ...
    try:
        if salt_path.exists():
            ...
            # a303 fix: previous version generated a fresh random
            # salt when the existing file's length was wrong (and
            # the FS was writable) — orphaning every Fernet
            # ciphertext encrypted with the OLD salt. User saw their
            # API keys silently disappear (decryption failed →
            # _fernet_read raises _FernetUnreadable → get_secret
            # returned None → "no provider configured"). Now: bad
            # length on an EXISTING file means use the legacy public-
            # derived salt (whose ciphertexts presumably still match)
            # rather than rotating to fresh-random.
            if len(salt) != 16:
                salt = b""
                legacy_fallback = True
        # First-install path only: generate + persist a fresh random
        # salt. Existing-file path falls through to legacy_fallback.
        if not salt and not legacy_fallback:
    ...
    if legacy_fallback or not salt:
        # a303 fix: warn once per process, not once per call.
        # _derive_key runs on EVERY get_secret / set_secret —
        # operators saw the same WARN line 5-10× per `huntova serve`
        # boot. Module-level sentinel suppresses repeats.
        if legacy_fallback and not getattr(_derive_key, "_legacy_warned", False):
            ...
            _derive_key._legacy_warned = True  # type: ignore[attr-defined]
