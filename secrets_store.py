    ...
    try:
        ...
        if not salt:
            ...
            try:
                ...
                # Read-only / non-POSIX fall-through. Use the salt
                # we just generated for THIS process; subsequent runs
                # will regenerate (Fernet decrypt will fail, the user
                # gets the existing _FernetUnreadable signal — better
                # than silent key drift).
                pass
    except Exception:
        # Any unexpected failure: legacy derivation so we don't block
        # the install entirely. Marked with a clear warning prefix so
        # operators see it in stderr.
        print("[secrets] WARN: salt-file read failed, falling back to legacy public derivation",
