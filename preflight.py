...
def _requirements_for(action: str, *, dry_run: bool = False) -> list[RequirementKey]:
    ...
    if dry_run:
        ...
        # Dry-run hunt walks setup wiring without making provider calls
        # or touching the lead pipeline — it's a smoke test, not an
        # actual hunt — so the ICP profile (used to score real candidates)
        # isn't required. AI_PROVIDER stays so the user still gets a
        # clear "no key configured" diagnostic when relevant.
        if action == "hunt":
            base = [r for r in base if r != RequirementKey.ICP_PROFILE]
    return base
