...
Built on top of Python's `questionary` for the interactive surface
(intro/outro/select/text/password/confirm/progress). Falls back to plain
`input()` when questionary isn't installed or when stdin isn't a TTY so
...
def print_banner(subtitle: str = "", show_tagline: bool = True) -> None:
    ...
    show_tagline: rotates a witty one-liner under the logo. Disable for
    non-onboarding surfaces where the tagline is noise.
    """
    if not _is_tty():
        return
    print()
...
def config_summary_card(items: list[tuple[str, str]], title: str = "Existing config detected") -> None:
    """Boxed key-value card showing existing config. items is a list of
    (label, value) pairs.
...
def detect_browser_open_support() -> tuple[bool, str]:
    """Return (can_open, reason). Detects SSH-without-display, WSL, and
    missing platform binaries so we don't try to open a browser when
