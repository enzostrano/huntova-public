...
def _safe_outbound_url(url: str) -> bool:
    ...
    Stricter than `url.lower().startswith(("http://","https://"))`, which
    can be fooled by case variants the parser later normalises differently
    from the prefix check. Now: parse first, allow only http/https, reject
    ...
    if not host:
        return False
    blocked_prefixes = ("127.", "10.", "169.254.", "192.168.")
    if host == "localhost" or host.startswith(blocked_prefixes):
        return False
