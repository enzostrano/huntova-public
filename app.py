...
def is_private_url(url: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        # DNS lookup — any resolved address in a private range disqualifies.
        try:
            infos = socket.getaddrinfo(h, None)
        except Exception:
            return True  # fail closed — refusing to fetch an unresolvable host is fine
        for info in infos:
            addr = info[4][0]
            try:
...
def gdpr_erasure(identifier):
    ...
    _atomic_write(ARCHIVED_JSON, archived)
    # Rebuild seen history from remaining leads
    global _seen_urls, _seen_fps
    _seen_fps = set()
    for l in leads:
        _seen_fps.add(make_fingerprint(l))
        _seen_fps.add(make_fingerprint_legacy(l))
    save_seen_history()
...
def _get_tier_model():
    ...
    Agency → Gemini Pro (smarter analysis, better emails)
...
def _dna_build_stage_1_prompt(wizard_data):
    ...
    feedback_good = wizard_data.get("_feedback_good", [])
    feedback_bad = wizard_data.get("_feedback_bad", [])
    feedback_ctx = ""
    if feedback_good:
        feedback_ctx += "\n\nPROVEN GOOD LEADS (user confirmed these were real prospects):\n"
        for l in feedback_good[:10]:
            feedback_ctx += f"- {l.get('org_name','?')} ({l.get('country','?')}) — {l.get('why_fit','')[:120]}\n"
    if feedback_bad:
        feedback_ctx += "\n\nCONFIRMED BAD LEADS (user said these were wrong):\n"
        for l in feedback_bad[:10]:
            feedback_ctx += f"- {l.get('org_name','?')} ({l.get('country','?')}) — {l.get('why_fit','')[:120]}\n"
...
def generate_agent_dna(wizard_data, feedback_good=None, feedback_bad=None, existing_dna=None):
    ...
    # Build blocklist of words that would find competitors
    service_words = set()
    for s in services:
    ...
    anti = strategy.get("anti_patterns", {})
    for sig in anti.get("competitor_signals", []):
        for w in sig.lower().split():
            if len(w) > 4:
                service_words.add(w)
    ...
    for item in raw_queries:
        ...
        # ── Strip search operators (SearXNG doesn't support them) ──
        ...
        # ── Check for remaining banned patterns ──
        _has_banned = False
...
# Stability fix (a910 — search abstraction audit):
# SearXNG circuit breaker. The previous search() code only caught
# Timeout / ConnectionError explicitly — every other failure (HTTP
# 5xx, HTTP 429, ChunkedEncodingError, JSONDecodeError) fell through
# to a bare `except Exception`. Worse, it had ZERO retries for any
# failure mode: one bad request → straight to DDG fallback. A flaky
# self-hosted SearXNG (502 during a restart, 429 from a public
# instance) would burn the whole hunt's quota by hammering DDG with
# 50–80 fallback queries, which itself rate-limits.
#
# This breaker tracks per-hunt SearXNG health. After
# SEARXNG_MAX_CONSEC_FAILURES consecutive failures, the breaker trips
# and search() goes straight to DDG for the rest of the hunt — once,
# loudly, with an SSE warning so the user knows the search backend
# degraded (instead of silently producing zero leads).
SEARXNG_MAX_CONSEC_FAILURES = 3
SEARXNG_MAX_RETRIES_PER_QUERY = 2  # per-query budget — bounded
SEARXNG_BASE_BACKOFF = 0.6  # seconds; doubled per attempt + jitter

class _SearxngBreaker:
    """Per-hunt circuit breaker for SearXNG. Reset on each hunt start
    via reset_search_breaker() — see run_agent()."""
    def __init__(self):
    ...
    def record_failure(self):
        ...
        if self.consec_failures >= SEARXNG_MAX_CONSEC_FAILURES:
            self.tripped = True
...
_searxng_breaker = _SearxngBreaker()


def reset_search_breaker():
    """Call at hunt start so a previous hunt's failures don't keep us
    in DDG-only mode forever."""
...
def _searxng_query_once(query, max_results, language, time_range, categories, attempt_timeout):
    """Single-shot SearXNG request. Returns (results_list, error_kind).
    error_kind is one of:
      None         — request succeeded (results may still be empty if
                     SearXNG returned 0 hits for this query)
      "timeout"    — connect or read timeout
      "conn"       — DNS/refused/SSL error
      "http_5xx"   — server-side outage (worth retrying)
      "http_4xx"   — client error / 404 (don't keep hammering)
      "decode"     — got a 200 but body wasn't JSON (instance reconfigured)
    """
    ...
    try:
        r = _search_session.get(_search_url, params=params, timeout=attempt_timeout)
        ...
        return [], "conn"
    # HTTP-level result classification — don't go through raise_for_status
    # so we can distinguish 5xx (retry) from 4xx (don't retry).
    if 500 <= r.status_code < 600:
        return [], "http_5xx"
    if r.status_code == 429:
        # Rate-limited: treat like 5xx so we back off and retry. If it
        # keeps happening the breaker will trip.
        return [], "http_5xx"
...
def search(query, max_results=MAX_RESULTS_PER_QUERY, language="en", time_range="", categories="general"):
    ...
    Retry policy (a910 — search abstraction audit):
    - Per-query retry budget: SEARXNG_MAX_RETRIES_PER_QUERY attempts
      on retryable failures (timeout, connection, 5xx, 429, JSON
      decode). 4xx (other than 429) skips retries — bad request,
      not flake.
    - Exponential backoff with jitter between attempts.
    - Per-hunt circuit breaker: after SEARXNG_MAX_CONSEC_FAILURES
      consecutive query-level failures, all subsequent calls skip
      SearXNG entirely and go straight to DDG fallback. A single SSE
      warning fires the first time the breaker trips so the user
      knows the search backend degraded (the previous version
      produced "zero leads" silently when SearXNG was down).
    ...
    # Circuit-breaker check: if a prior burst of SearXNG failures
    # already tripped the breaker for this hunt, skip the SearXNG
    # round-trip entirely and go straight to DDG.
    if _searxng_breaker.is_tripped():
        if not _searxng_breaker.warned:
            _searxng_breaker.warned = True
            try:
                emit_log(
                    "Search backend degraded — SearXNG unreachable, "
                    "using DuckDuckGo fallback for the rest of this "
                    "hunt. Lead volume + freshness will be lower.",
                    "warn",
                )
            except Exception:
                pass
    ...
    for attempt in range(SEARXNG_MAX_RETRIES_PER_QUERY + 1):
        if _check_stop():
            return []
        # Bounded timeout per attempt — slightly longer on retries.
        attempt_timeout = 6 + (attempt * 2)
        results, err = _searxng_query_once(
            query, max_results, language, time_range, categories, attempt_timeout,
        )
        if err is None:
            _searxng_breaker.record_success()
            if results:
                return results
            # Got a valid 200 with zero hits — query just doesn't
            # match. Try DDG once as a peer search engine but don't
            # count it as a SearXNG failure.
            return _ddg_fallback_search(query, max_results)

        last_error_kind = err

        # 4xx (non-429) is a permanent failure for THIS query — bail
        # without retries but DON'T trip the breaker (might be a
        # malformed query, not an outage).
        if err == "http_4xx":
            try:
                emit_log(f"SearXNG rejected query (4xx): {query[:50]}", "warn")
            except Exception:
                pass
            return _ddg_fallback_search(query, max_results)

        # Retryable: back off (with jitter) before the next attempt.
        if attempt < SEARXNG_MAX_RETRIES_PER_QUERY:
            try:
                import random as _r
                backoff = SEARXNG_BASE_BACKOFF * (2 ** attempt) + _r.uniform(0, 0.3)
                time.sleep(backoff)
            except Exception:
                time.sleep(SEARXNG_BASE_BACKOFF)

    # Exhausted retries → record breaker failure + fall back to DDG.
    _searxng_breaker.record_failure()
    try:
        if last_error_kind == "timeout":
            emit_log(f"Search timeout (after retries) for: {query[:50]}", "warn")
        elif last_error_kind == "conn":
            emit_log("SearXNG unreachable (after retries) — falling back to DuckDuckGo", "info")
        elif last_error_kind == "http_5xx":
            emit_log("SearXNG returned 5xx (after retries) — falling back to DuckDuckGo", "warn")
        elif last_error_kind == "decode":
            emit_log("SearXNG returned malformed JSON — JSON API may be disabled", "warn")
        else:
            emit_log(f"Search error ({last_error_kind}) — falling back to DuckDuckGo", "warn")
    except Exception:
        pass

    # If this failure tipped the breaker over the threshold, surface
    # the SSE warning right now (rather than waiting for the next
    # query). This is the user-visible signal that the hunt is now
    # running degraded.
    if _searxng_breaker.is_tripped() and not _searxng_breaker.warned:
        _searxng_breaker.warned = True
        try:
            emit_log(
                "Search backend degraded — SearXNG failed "
                f"{_searxng_breaker.consec_failures} times in a row. "
                "Switching to DuckDuckGo fallback for the rest of this "
                "hunt. Lead volume + freshness will be lower.",
                "warn",
            )
        except Exception:
...
def crawl_prospect(url, max_subpages=4):
    ...
    def _fetch(u):
        try:
            if _check_stop(): return "", ""
            r = requests.get(u, headers=_ua, timeout=10, verify=False)
        ...
        if _check_stop(): return "", ""
        try:
            jr = requests.get(f"https://r.jina.ai/{u}", headers={"Accept": "text/plain", "User-Agent": _ua["User-Agent"]}, timeout=15)
            if jr.status_code == 200 and len(jr.text) > 100:
                jt = re.sub(r'!\[[^\]]*\]\([^)]+\)', '', jr.text)
                jt = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', jt)
                jt = re.sub(r'#+\s*', '', jt)
...
def run_agent():
    ...
    # Reset the per-hunt SearXNG circuit breaker (a910 — search
    # abstraction audit). A previous hunt's failures must not leave
    # this hunt stuck in DDG-only mode.
    reset_search_breaker()
    _has_searxng = check_searxng()
    if _has_searxng:
        emit_log(f"Search engine connected", "ok")
    else:
        # a811: previously this returned early with "cannot start" —
        # but search() has a working DuckDuckGo HTML fallback, so a
        # refusal to start was a self-inflicted outage. Pre-trip the
        # breaker so subsequent search() calls go straight to DDG
        # without paying retries up front, surface a single warning,
        # and continue.
        _searxng_breaker.tripped = True
        _searxng_breaker.warned = True
        emit_log(
            "SearXNG unreachable at hunt start — running in degraded "
            "mode via DuckDuckGo fallback. Lead volume + freshness "
