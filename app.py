...
def _generate_brain_queries(brain, countries, max_queries=50):
    ...
    # a368 fix (BRAIN-7): normalise to list-of-dicts before iterating.
    # Was: `for good in (goods or [])[:3]; good.get("name","")` which
    # crashed with `'str' object has no attribute 'get'` whenever the
    # wizard supplied the natural string-from-textarea shape (the
    # `example_good_clients` question is a textarea). Same bug class
    # as a331's _generate_training_dossier crash; that fix added
    # _normalize_examples_top but never reached this function.
    def _norm_examples(raw):
        if not raw:
            return []
        if isinstance(raw, str):
            pieces = []
        ...
        if isinstance(raw, list):
            ...
            return out
        return []
    for good in _norm_examples(goods)[:3]:
        name = good.get("name", "")
        if name and len(name) > 2:
