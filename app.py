...
def enrich_contact(page, base_url, lead):
    ...
    if all_emails:
        ...
        if _personal:
            # validate_email can return None for invalid TLD / disposable
            # domain. Mirror the generic branch's null-check so a bad
            # candidate doesn't poison contact_email with None and skip
            # the generic-fallback path below.
            _valid_personal = validate_email(_personal[0])
            if _valid_personal:
