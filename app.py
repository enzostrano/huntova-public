...
def run_agent():
    ...
    if ctx and _wiz_data.get("company_name"):
        ...
        try:
            ...
            # a340 fix: was logged at "debug" level — most users never
            # saw it. Bumped to "warn" so the failure is visible in the
            # agent log + sidebar pill. Includes the exception type and
            # a hint at what to check.
            _gen_err_str = str(_gen_err)[:200]
            _hint = ""
            if "401" in _gen_err_str or "unauthorized" in _gen_err_str.lower():
                _hint = " — check API key in Settings → Engine."
            elif "402" in _gen_err_str or "credit" in _gen_err_str.lower() or "insufficient" in _gen_err_str.lower():
                _hint = " — your AI provider account is out of credits."
            elif "429" in _gen_err_str or "rate" in _gen_err_str.lower():
                _hint = " — provider rate-limited; wait 60s and retry."
            emit_log(
                f"DNA generation failed: {type(_gen_err).__name__}: {_gen_err_str}{_hint}",
                "warn",
            )
    # Cache DNA on ctx
    if ctx:
