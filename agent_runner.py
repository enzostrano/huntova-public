...
class _SubagentRegistry:
    ...
    def register(self, sa: _Subagent) -> bool:
        with self._lock:
            ...
            # a299: honor `max_concurrent_research` setting if set —
            # was previously ignored. Range-clamped to [1, 10] inside
            # the settings POST allowlist.
            try:
                from runtime import is_local
                from db_driver import is_sqlite as _is_sql
            except Exception:
                _is_sql = None
            _user_cap = _SUBAGENT_MAX_PER_USER
            try:
                # Read settings on the fly via app.load_settings (works
                # in both local single-user and cloud paths).
                from app import load_settings as _ls
                _s = _ls()
                _override = _s.get("max_concurrent_research")
                if _override:
