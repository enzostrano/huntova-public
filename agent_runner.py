...
def spawn_subagent(user_id: int, kind: str, payload: dict | None = None) -> dict:
    ...
    def _runner():
        ...
        if _push is not None:
            try:
                _push(sa.payload.get("provider"))
                _override_set = True
            except Exception as _ov_err:
                # Override couldn't be set — log so we can debug.
                # _push reference is still good; finally will clear.
                print(f"[subagent {sa.id}] push_provider_override raised: {type(_ov_err).__name__}: {_ov_err}", flush=True)
