...
class AgentRunner:
    ...
    async def start_agent(self, user_id: int, user_email: str, user_tier: str,
        ...
        # Provider pre-flight: cheap, sync — just enumerates configured
        # keys (no AI call). If zero providers, fail fast with a clear
        # message so the user sees a toast instead of a silent crash
        # mid-hunt. Local-mode-only since cloud has its own provider
        # routing.
        try:
            ...
            if _CAPS.mode == "local":
                ...
                if not (list_available_providers() or []):
                    return {
                        "ok": False,
                        "error": "No AI provider configured. Open Settings → Providers to add a key.",
                    }
        except Exception:
            # If the pre-flight itself blows up, let the agent thread try
            # — the original behavior. Don't fail the whole start path.
            pass

        # Health check removed from async start path — blocking sync AI call
        # inside async context causes hangs. Agent thread handles failures
        ...
        with self._lock:
            ...
            # `int("5.0")` raises ValueError → silently coerced to None
            # (unlimited), losing the user's intent. Go through float()
            # first so `"5"`, `"5.0"`, `5`, `5.0` all land at int(5).
            _max_leads = config.get("max_leads")
            try:
                _max_leads = int(float(_max_leads)) if _max_leads not in (None, "", 0) else None
            ...
            try:
                _timeout_min = int(float(_timeout_min)) if _timeout_min not in (None, "", 0) else None
            ...
            ctx.agent_config = {
                ...
                # Recipe name flows through so the recipe-adapter +
                # adaptation-rules plugins can hydrate
                # HV_RECIPE_ADAPTATION on web hunts (a11 round-10
                # feature was wired wrong — see a192 fix below).
                "recipe_name": (config.get("recipe_name") or "").strip()
    ...
    def stop_agent(self, user_id: int):
        ...
        If the user is queued (not yet running), drop the queue slot
        so they aren't stuck waiting for a hunt they cancelled.

        a256: emit a "Stopping…" status + log line immediately so the
        UI surfaces feedback within the SSE round-trip. The actual
        agent thread checks _check_stop() at the next iteration which
        can be 5-30s later (mid-fetch / mid-AI-call), and users were
        complaining the Stop button "did nothing".
        """
        ...
        with self._lock:
            try:
                ...
                pass
        try:
            ctx.emit_log("Stop requested by user — finishing current task…", "warn")
            ctx.emit_status("Stopping…", "stopping")
        except Exception:
            pass
    ...
    def _run_agent_thread(self, user_id: int):
        ...
        try:
            ...
            # Stability fix (audit wave 25): the original line read
            # `(config or {}).get(...) if isinstance(config, dict) else None`
            # but `config` was never bound in this method's scope —
            # `start_agent` takes `config` as a parameter and stashes
            # it on `ctx.agent_config`, but never plumbed it through to
            # `_run_agent_thread`, which only takes (self, user_id).
            # Every web hunt + every CLI `huntova hunt` raised
            # NameError on this line, control jumped to the outer
            # `except Exception` at line ~344, the "Agent crashed"
            # handler ran, and `run_agent_scoped(ctx)` never executed
            # — so no leads, no SSE updates beyond the crash log,
            # nothing. Live since v0.1.0a11.
            #
            # Read from `ctx.agent_config` instead, which `start_agent`
            # always populates (now including `recipe_name`).
            ...
            # Clear any pending pause/stop ctrl so a "stop" or "pause"
            # set during a thread's last-millisecond exit doesn't leak
            # into the next agent run for the same user. start_agent +
            # _process_queue both reset this defensively before
            # spawning, but resetting here too keeps the invariant
            # "an agent that has finished holds no pending ctrl".
            try:
                with ctx.agent_ctrl_lock:
                    if isinstance(ctx.agent_ctrl, dict):
                        ctx.agent_ctrl["action"] = None
            except Exception:
                pass
            ctx.agent_running = False
    ...
    def _process_queue(self):
        ...
        with self._lock:
            while self._queue and len(self._running) < MAX_CONCURRENT_AGENTS:
                ...
                # Mirror start_agent's start-failure cleanup. If thread.start()
                # raises (OS-level thread limit, signal mask issues), self._running
                # would otherwise hold a stale Thread object that .is_alive()
                # returns False for, blocking the user's next attempt.
                try:
                    self._running[next_user_id] = thread
                    thread.start()
                    started.append(("started", ctx))
                except Exception:
                    self._running.pop(next_user_id, None)
                    ctx.agent_running = False
                    raise
...
# ─────── Multi-agent / sub-agent runtime ────────────────────────────────
#
# A "subagent" is a short-lived background task spawned alongside (or
# without) the main hunt — e.g. deep-research a top-3 lead while the
# main hunt keeps surfacing new prospects, or re-score the current
# qualified pool after a rule edit. They run in their own daemon
# threads, stream status to the same UserEventBus the dashboard already
# subscribes to (no new SSE plumbing), and are visible/cancelable from
# the dashboard's "Agents" panel.
#
# Concurrency cap is intentionally low (3 per user) because every
# spawned subagent burns the user's BYOK token budget. Cheap routing —
# subagents pick up the same provider config the main agent uses, no
# extra fan-out.

import uuid as _uuid

_SUBAGENT_MAX_PER_USER = 3
_SUBAGENT_KINDS = ("deep_research", "inbox_scan")  # a295: removed qualify_pool — was a stub returning error


class _Subagent:
    __slots__ = ("id", "user_id", "kind", "payload", "status",
                 "started_at", "finished_at", "result", "error", "cancel_event")

    def __init__(self, user_id: int, kind: str, payload: dict):
    ...
    def to_dict(self) -> dict:
        return {
            "id": self.id, "user_id": self.user_id, "kind": self.kind,
...
class _SubagentRegistry:
    def __init__(self):
        self._by_id: dict[str, _Subagent] = {}
    ...
    def list_user(self, user_id: int) -> list[dict]:
        with self._lock:
            ...
            return [self._by_id[i].to_dict() for i in ids if i in self._by_id]

    def get(self, sub_id: str) -> _Subagent | None:
        with self._lock:
            return self._by_id.get(sub_id)

    def register(self, sa: _Subagent) -> bool:
        # a303 fix: read max_concurrent_research BEFORE acquiring
        # `self._lock`. a299's wire did the file-read inside the lock,
        # which serialised every register/cancel call behind disk I/O
        # of every other user's settings load. Now the I/O happens
        # without contention, the actual cap-check is trivial inside
        # the lock.
        _user_cap = _SUBAGENT_MAX_PER_USER
        try:
            from app import load_settings as _ls
            _override = _ls().get("max_concurrent_research")
            if _override:
                _user_cap = max(1, min(int(_override), 10))
        except Exception:
            pass
        with self._lock:
            # Prune finished entries older than 10 min so the panel
            # doesn't accumulate forever without an explicit clear.
            #
            # Stability fix (audit wave 23): the previous version only
            # rebuilt _by_user[uid] but never deleted from _by_id, so
            # every finished subagent's _Subagent object (with its
            # payload + result dicts — deep_research stuffs a 14-page
            # crawl result in there) lived forever in the process.
            # Now drop from _by_id first, sweeping ALL users not just
            # the calling one, so a low-activity user's subagents
            # don't leak indefinitely waiting for them to call register
            # again.
            cutoff = time.time() - 600
            stale_ids = [
                i for i, s in self._by_id.items()
                if s.finished_at is not None and s.finished_at <= cutoff
            ]
            for i in stale_ids:
                self._by_id.pop(i, None)
            # Rebuild every per-user list to drop dangling references.
            for uid, lst in self._by_user.items():
                lst[:] = [i for i in lst if i in self._by_id]
            ids = self._by_user.setdefault(sa.user_id, [])
            running = sum(1 for i in ids
                          if self._by_id.get(i) and self._by_id[i].status in ("starting", "running"))
            # _user_cap was computed above (a303 fix — settings read
            # outside the lock to avoid serialising disk I/O).
            if running >= _user_cap:
                return False
            self._by_id[sa.id] = sa
            ids.append(sa.id)
            return True

    def cancel(self, user_id: int, sub_id: str) -> bool:
        with self._lock:
            sa = self._by_id.get(sub_id)
            if not sa or sa.user_id != user_id:
                return False
            sa.cancel_event.set()
            # a277: was `if sa.status == "starting":` only — for a subagent
            # in "running" state, the cancel_event was set but status
            # stayed "running" until the runner thread eventually noticed
            # and exited. The `register()` slot count at line 615-617
            # treats "running" as occupied, so a user who cancelled all 3
            # of their concurrent slots would be unable to spawn a new
            # subagent for up to ~10 minutes (until the cutoff sweep
            # cleared stale entries with a finished_at). Now ANY non-
            # terminal status flips to "cancelled" + sets finished_at,
            # freeing the slot for a fresh spawn even if the runner thread
            # hasn't noticed the cancel_event yet. The runner thread's
            # final-state assignment at agent_runner.py:711 (`sa.status =
            # "cancelled" if sa.cancel_event.is_set() else "done"`) is
            # idempotent — it'll re-confirm "cancelled" when it lands.
            _TERMINAL = {"done", "error", "cancelled"}
            if sa.status not in _TERMINAL:
                sa.status = "cancelled"
...
def _emit_sa(ctx, sa: _Subagent) -> None:
    """Push a snapshot of the subagent over the user's SSE bus so the
    dashboard's Agents panel can refresh without polling."""
    try:
        ctx.bus.emit("subagent_status", sa.to_dict())
    except Exception:
        pass


def spawn_subagent(user_id: int, kind: str, payload: dict | None = None) -> dict:
    """Public entrypoint. Returns {ok, id?, error?}.

    Each kind has its own runner — the registry just tracks state.
    Token usage is bounded by the per-kind work-cap (e.g. deep_research
    crawls at most 14 pages per its existing cap), so a user can't
    spawn three subagents and burn 30× their hunt cost.
    """
    if kind not in _SUBAGENT_KINDS:
        return {"ok": False, "error": f"unknown kind: {kind!r}"}
    sa = _Subagent(user_id=user_id, kind=kind, payload=payload or {})
    if not subagent_registry.register(sa):
        return {"ok": False, "error": f"max {_SUBAGENT_MAX_PER_USER} concurrent subagents per user"}
    ctx = get_or_create_context(user_id)
    _emit_sa(ctx, sa)
    ...
    def _runner():
        sa.status = "running"
        _emit_sa(ctx, sa)
        # Per-subagent provider override. If `payload.provider` was set
        # (from the chat AI selector or the spawn_agents fan-out), pin
        # this thread's get_provider() to that slug so the subagent
        # uses a different AI than the main hunt.
        #
        # a277: split the import from the call. Pre-a277 they shared a
        # try/except — if `_push(slug)` raised (e.g. a future validator
        # that rejects unknown slugs), the bare `except` clobbered
        # `_push = None` and the `finally` block's clearing call at
        # line ~750 silently became `if _push is not None: _push(None)`
        # → no-op, leaving the override in place for whatever thread
        # the worker pool recycled this subagent's thread to next. The
        # split below preserves `_push` (reference to the function)
        # even when the call itself raises, so the cleanup ALWAYS runs
        # if we successfully imported.
        _push = None
        _override_set = False
        try:
            from providers import push_provider_override as _push_fn
            _push = _push_fn
        except Exception:
            pass
        if _push is not None:
            _wanted = sa.payload.get("provider")
            try:
                _push(_wanted)
                _override_set = True
            except Exception as _ov_err:
                # a285 fix: contract violation, not recoverable. The
                # caller asked for provider X (e.g. cheap Groq for a
                # research crawl); silent fallback to the default
                # provider would burn the user's preferred-AI budget on
                # the wrong key. Mark the subagent failed and emit a
                # status so the user actually sees it. Was print() →
                # invisible to anyone watching the dashboard SSE feed.
                _err_msg = (f"could not pin provider {_wanted!r}: "
                            f"{type(_ov_err).__name__}: {_ov_err}")
                print(f"[subagent {sa.id}] {_err_msg}", flush=True)
                try:
                    sa.status = "error"
                    sa.error = _err_msg
                    sa.finished_at = time.time()  # frees slot at next register()
                    _emit_sa(ctx, sa)
                except Exception:
                    pass
                return
        # a245: bind cancel_event into app's thread-local so _check_stop()
        # consults it. crawl_prospect calls _check_stop() per page; this
        # makes "Cancel" in the Agent panel actually halt the 14-page
        # research crawl mid-flight instead of letting it finish all
        # pages before honoring the cancel.
        try:
            from app import _tl as _app_tl
            _app_tl.subagent_cancel_event = sa.cancel_event
        except Exception:
            _app_tl = None
        try:
            if kind == "inbox_scan":
                from cli_inbox import _scan_inbox
                since = int(sa.payload.get("since_days") or 7)
                res = asyncio.run(_scan_inbox(user_id, since_days=since))
                sa.result = res or {}
            elif kind == "deep_research":
                # `cmd_research` in cli.py runs the 14-page crawl + rewrite
                # against a single lead_id. The CLI flow works synchronously
                # against our DB, so we re-use it here. payload: {lead_id}.
                # If the user cancels mid-crawl, the cancel_event will be
                # honored at the next ctx.bus.emit boundary.
                from cli import cmd_research as _cmd_research  # noqa: F401
                import argparse as _ap
                lead_id = sa.payload.get("lead_id")
                if not lead_id:
                    raise ValueError("lead_id is required")
                # Build a synthetic argparse namespace covering every
                # field cmd_research reads — missing ones cause silent
                # int(None) failures or skip the subagent's payload.
                #
                # a277: clamp `pages` to the documented 14-page cap. The
                # downstream `cmd_research` clamps to 25 (cli.py:3159)
                # but the docstring at line 707 advertises "14-page
                # crawl" and a hostile payload `{"pages": 999}` would
                # blow through that into a 25-page burst on the user's
                # BYOK token budget. Clamping here at 14 keeps the cap
                # honest. Users who want more crawl depth call
                # cmd_research directly via CLI, where 25 still applies.
                _pages_req = int(sa.payload.get("pages") or 14)
                _pages_cap = max(1, min(_pages_req, 14))
                ns = _ap.Namespace(
                    lead_id=str(lead_id),
                    pages=_pages_cap,
                    tone=str(sa.payload.get("tone") or ""),
                    batch=int(sa.payload.get("batch") or 0),
                    above=float(sa.payload.get("above") or 0.0),
                    json=True, dry_run=False,
                )
                _cmd_research(ns)
                sa.result = {"ok": True, "lead_id": lead_id}
            elif kind == "qualify_pool":
                # Stub for now — the rescore-existing-pool path doesn't have
                # a stable sync entrypoint yet; surface a clear status so
                # the dashboard treats it as an obvious "TODO" rather than
                # silent dead air.
                sa.error = "qualify_pool: re-scoring pipeline not exposed yet — coming next release"
                sa.status = "error"
                sa.finished_at = time.time()
                _emit_sa(ctx, sa)
                return
            # Stability fix (audit wave 27): cancel() at line ~617
            # only flips `status="cancelled"` when status is still
            # "starting"; for a running subagent it just sets the
            # cancel_event and leaves status alone, so the unconditional
            # `sa.status = "done"` overwrote a cancellation the user had
            # already requested. The "cancelled" terminal state was
            # unreachable for any subagent that had progressed past
            # the runner's first line. Reflect cancel_event explicitly.
            sa.status = "cancelled" if sa.cancel_event.is_set() else "done"
        except Exception as e:
            sa.status = "error"
            sa.error = f"{type(e).__name__}: {str(e)[:160]}"
        finally:
            # Clear the thread-local override so a recycled thread (if
            # ever) doesn't inherit a previous subagent's slug.
            # a277: only clear if we successfully SET the override —
            # avoids re-firing the validator on cleanup that just
            # rejected the same slug a moment ago. The `_push` ref
            # itself is preserved across the try at line 700ff so
            # this block always runs when the import succeeded.
            try:
                if _push is not None and _override_set:
                    _push(None)
            except Exception:
                pass
            try:
                if _app_tl is not None:
                    _app_tl.subagent_cancel_event = None
            except Exception:
                pass
            sa.finished_at = time.time()
            _emit_sa(ctx, sa)

    t = threading.Thread(target=_runner, name=f"hv-sa-{sa.id}", daemon=True)
    t.start()
