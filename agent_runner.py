...
class AgentRunner:
    ...
    def _run_agent_thread(self, user_id: int):
        ...
        try:
            ...
            # Import and run the agent. Round-11 finding #2: the
            # `from app import` was outside the try/finally, so an
            # ImportError (circular import, syntax error from a
            # hot-patch, transient module-load failure) would skip the
            # finally and leave HV_RECIPE_ADAPTATION set in os.environ
            # — every subsequent agent run on this process would
            # inherit the stale adaptation JSON for the wrong recipe.
            try:
