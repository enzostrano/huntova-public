...
Specialised for Huntova's lead schema. Wired in cli.py via
`register(sub)`. Uses stdlib `csv` only — no new deps.
...
_HEURISTIC = [
    # Stability fix (audit wave 27): contact_linkedin must come BEFORE
    # org_website. Headers like "LinkedIn URL" / "LinkedIn Profile URL"
    # contain "url" — which the org_website needles match — so the
    # previous order claimed the column for org_website and never
    # populated contact_linkedin. Generic Clay/custom CSV imports
    # with that header silently lost LinkedIn data AND clobbered
    # org_website with profile URLs. The order of _HEURISTIC entries
    # is significant: first-match-wins per header.
...
def _dedup_keys(lead: dict) -> tuple:
    ...
    email = (lead.get("contact_email") or "").strip().lower()
    # Without an email, two people with different names at the same
    # company would collide on (site, "") and one would be silently
    # dropped. Fall back to contact_name to keep them distinct.
    name = (lead.get("contact_name") or "").strip().lower() if not email else ""
    return (site, email, name)
...
def _import(user_id: int, args: argparse.Namespace,
    ...
    with fh:
        ...
        print(msg + "\n")
        # Zero rows imported is almost never the user's intent. Common
        # causes: CSV header case mismatch, every row already in the DB
        # (skipped as duplicate), or a wrong --source flag. Exit non-zero
        # so callers / shell loops notice instead of treating it as a
        # silent success.
        if imported == 0:
            print("[huntova] no valid rows imported", file=__import__("sys").stderr)
            return 1
...
def register(sub) -> None:
    ...
    m = sub.add_parser("migrate",
        ...
        description="Bulk-import leads from CSV exports (Apollo / Clay / Hunter / generic).")
