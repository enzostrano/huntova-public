...
var _hvAccount=null;
var _hvFeatures={};
// Runtime capabilities. Resolved by /api/runtime on first paint. In
// local mode billing/auth/upgrade surfaces hide. Defaults to cloud
// shape so the page works even if the fetch lags.
...
let evtSrc=null;const agLeads=[];var _sseRetry=1000;var _leadRenderTimer=null;var _sseConnecting=false;var _sseRetryTimer=null;
...
function connectSSE(){
  ...
  // Stability fix (audit wave 27): cancel any pending retry timer
  // before opening a new connection. Without this, onerror/catch
  // schedule setTimeout(connectSSE, retry) AND visibilitychange can
  // call connectSSE() directly when the tab becomes visible. Both
  // create new EventSource objects whose listeners (registered below)
  // are never removed from the previous instance — duplicate streams
  // doubled every `lead` event into agLeads + double-rendered the
  // CRM. Centralise timer clearance here.
  ...
  fetch('/api/status').then(function(r){if(r.status===401){_sseConnecting=false;window.location.href='/landing';return}
    try{evtSrc=new EventSource('/agent/events');_sseConnecting=false}catch(e){_sseConnecting=false;_sseRetry=Math.min(_sseRetry*2,30000);if(_sseRetryTimer)clearTimeout(_sseRetryTimer);_sseRetryTimer=setTimeout(connectSSE,_sseRetry);return}
  ...
  evtSrc.addEventListener('dna_updated',function(e){try{
    ...
    evtSrc.onerror=()=>{if(evtSrc){try{evtSrc.close()}catch(_){}}evtSrc=null;_sseRetry=Math.min(_sseRetry*2,30000);if(_sseRetryTimer)clearTimeout(_sseRetryTimer);_sseRetryTimer=setTimeout(connectSSE,_sseRetry)};
  }).catch(function(){_sseConnecting=false;_sseRetry=Math.min(_sseRetry*2,30000);if(_sseRetryTimer)clearTimeout(_sseRetryTimer);_sseRetryTimer=setTimeout(connectSSE,_sseRetry)});
}
