...
function hvLoadAccount(){
  fetch('/api/account').then(function(r){return r.json()}).then(function(d){
    ...
    if(umdTier)umdTier.textContent=(_hvAccount.tier||'free').toUpperCase();
    ...
    if(_hvAccount.email_verified===false){
...
function renderRow(l,i,isFresh){
  ...
  <div class="org-c"><div class="org-n">${esc(l.org_name||'Unknown')}</div><div class="org-w">${l.org_website?`<a href="${esc(l.org_website)}" target="_blank" onclick="event.stopPropagation()">website</a>`:''}</div></div>
...
function huntNarrate(icon,text,sub,key){
  ...
  var now=Date.now();
  if(key&&_huntNarr.lastKey===key&&(now-_huntNarr.lastNarrTs)<350)return;
  _huntNarr.lastKey=key||'';_huntNarr.lastNarrTs=now;
...
function connectSSE(){
  ...
  evtSrc.addEventListener('crm_refresh',()=>loadCRM());
  evtSrc.addEventListener('credits_exhausted',()=>{hvLoadAccount();hvTokenPopup('empty')});
...
function showRandomTip(){
  let idx;do{idx=Math.floor(Math.random()*neoTips.length)}while(idx===lastTipIdx);
  lastTipIdx=idx;var _nm=$('neoMsg');if(_nm)_nm.innerHTML='<b class="neo-tip">Tip:</b> '+esc(neoTips[idx]);
}
...
function addAgentLeadRow(l){
  ...
  tr.innerHTML=`<td><span class="sc ${_agSc}">${s}</span></td>
    ...
    <td>${l.org_linkedin?'<a href="'+esc(l.org_linkedin)+'" target="_blank" class="ag-link">LinkedIn</a>':'—'}</td>`;
...
(function(){
  ...
  var hi=$('dashHi');
  if(hi&&_hvAccount&&_hvAccount.display_name){
...
function openLeadModal(lid){
  ...
  h+='<div class="si-field"><div class="si-label">Website</div><div class="si-val">'+(l.org_website?'<a href="'+esc(l.org_website)+'" target="_blank">'+esc(l.org_website).replace('https://','').replace('http://','')+'</a>':'<span class="emp">—</span>')+'</div></div>';
  ...
  if(hasEmail){
    ...
    if(rwH.length>0){
      ...
      rwH.slice().reverse().forEach(function(rh,ri){
        h+='<div class="lm-eh-item" onclick="revertEmail(\''+id+'\','+ri+')" title="Click to restore this version">';
  ...
  h+='<button class="research-btn" id="rb-'+id+'" onclick="startResearch(\''+id+'\')" data-feature="research">🔬 Deep Research<span class="research-cost">(1 credit)</span></button>';
...
h+='<div class="lm-sec">';
  ...
  h+='<textarea id="mn-'+id+'" class="lm-notes" placeholder="Add notes about this lead...">'+esc(l.notes||'')+'</textarea>';
  h+='<button class="lm-nbtn" onclick="saveModalNotes(\''+id+'\')">Save Notes</button>';
  h+='</div>';
...
function renderLeadPage(l) {
  ...
  if (l.org_website) h += '<div class="ldp-field"><span class="ldp-field-label">Website</span><a href="' + esc(l.org_website) + '" target="_blank">' + esc(l.org_website) + '</a></div>';
  if (l.contact_linkedin || l.org_linkedin) h += '<div class="ldp-field"><span class="ldp-field-label">LinkedIn</span><a href="' + esc(l.contact_linkedin || l.org_linkedin) + '" target="_blank">Profile</a></div>';
  h += '</div>';
  ...
  (function(){
    ...
    function _pgFlush(){
      ...
      fetch('/api/update',{
        ...
        body:JSON.stringify({lead_id:id,email_subject:ss||currentModalLead.email_subject,email_body:sb||currentModalLead.email_body})
      }).then(function(r){
...
function startResearch(lid){
  ...
  // Pre-flight credit check — avoid making the user confirm a spend they can't afford
  ...
  if(!confirm('Deep Research costs 1 credit. This will re-scrape the prospect\'s website, do an AI deep-analysis, and rewrite the email.\n\nProceed?'))return;
  _researchActive=lid;
...
async function saveSettings(){
  try{
    const body={booking_url:$('sBooking').value.trim(),from_name:$('sName').value.trim(),
      from_email:$('sEmail').value.trim(),phone:$('sPhone').value.trim(),website:$('sWebsite').value.trim()};
    const r=await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    const d=await r.json();
    if(d.ok){toast('✅ Settings saved');closeSettings()}else{toast('Error: '+(d.error||'unknown'))}
...
function launchHuntQuickStart(){
  ...
  // Credit gate, mirrors openStartPopup logic.
  var cr=_hvAccount?_hvAccount.credits_remaining:0;
...
function launchAgent(){
  ...
  try{
    ...
    if(huntsBtn)huntsBtn.click();
    huntReset();huntShowState('active');huntSetCurrent('Launching hunt in '+sel.length+' countries...');
    addLog('Starting Huntova in '+sel.length+' countries — will run until stopped or credits exhausted','info');
    ...
    fetch('/agent/control',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({action:'start',countries:sel})})
      .then(function(r){return r.json()}).then(function(d){
        ...
        // If the user is out of credits, open pricing so they have a one-click path.
        var msg=d.error||'Could not start';
        toast(msg);
        if(/credit|upgrade/i.test(msg)){try{hvOpenPricing()}catch(_){}}
        huntShowState('idle');
      })
      .catch(function(e){
        _launchInFlight=false;
...
function iwSendAssist(){
  ...
  msgs.innerHTML+='<div class="iwiz-chat-msg bot" id="'+loadId+'"><span style="color:var(--t3)">Thinking with Gemini Pro...</span></div>';
  msgs.scrollTop=msgs.scrollHeight;
