#!/usr/bin/env bash
# Huntova one-shot installer (v0.1.0a12).
...
if pipx list --short 2>/dev/null | awk '{print $1}' | grep -qx huntova; then
  pipx upgrade huntova >/dev/null 2>&1 || \
...
if [ -t 0 ] && [ -t 1 ]; then
  ...
  chat "no terminal attached (curl | sh) — opening the web wizard for you..."
  echo
  port=5050
  # Spawn the server detached so this script can finish cleanly.
  nohup huntova serve --no-browser --port "$port" \
    >"$HOME/.local/share/huntova/install-launch.log" 2>&1 &
  # Wait up to 8 seconds for the server to start binding.
  ...
  case "$(uname -s 2>/dev/null)" in
    Darwin) open "$url" 2>/dev/null && ok "browser opened at $url" || warn "couldn't open browser — visit $url manually" ;;
    Linux)
      if command -v xdg-open >/dev/null 2>&1; then
        xdg-open "$url" 2>/dev/null && ok "browser opened at $url" || warn "couldn't open — visit $url manually"
      elif command -v wslview >/dev/null 2>&1; then
        wslview "$url" 2>/dev/null && ok "browser opened at $url" || warn "visit $url manually"
      else
  ...
  echo
  printf "  %sserver running in background (PID $!).%s\n" "$DIM" "$RESET"
  printf "  %sstop with: huntova daemon stop  ·  or kill $!%s\n" "$DIM" "$RESET"
  echo
fi
