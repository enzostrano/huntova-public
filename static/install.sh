...
if [ -t 0 ] && [ -t 1 ]; then
  ...
  # curl | sh path — no TTY. Earlier versions tried to auto-spawn
  # `huntova serve` and open the browser, but stale processes on
  # :5050 + race-with-server-bind caused "page not found" in the
  # browser before the new server was ready. Pro pattern (Homebrew,
  # Rust, OpenClaw): install ends, user runs the next command in
  # their terminal. Single deterministic step, zero race conditions.
  echo
  printf "  %s▸%s One more command:\n" "$CYAN" "$RESET"
  echo
  printf "      %shuntova onboard%s\n" "$BOLD" "$RESET"
  echo
  printf "  %s(picks a provider, saves your key, opens the dashboard)%s\n" "$DIM" "$RESET"
  echo
fi
