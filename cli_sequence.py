...
async def _run_once(user_id: int, dry_run: bool, max_send: int) -> dict:
    ...
    for ld in leads or []:
        ...
        try:
            from email_service import _send_email_sync, SMTPDeliveryError, SMTPRateLimitedError
            html = ("<pre style='font-family:inherit;white-space:pre-wrap;font-size:14px'>"
                    + body.replace("<", "&lt;").replace(">", "&gt;") + "</pre>")
            # a302: mint the HTTPS one-click unsubscribe URL for this
            # (user, lead). Server-side import is fine — cli_sequence
            # is run from a CLI with the same Python path as server.py.
            _unsub = None
            try:
                from server import unsub_url as _unsub_fn
                _unsub = _unsub_fn(user_id, lead_id)
            except Exception:
                pass
            msg_id = _send_email_sync(to, subject, html, plain_body=body,
                                       unsub_url=_unsub)
        except SMTPRateLimitedError as e:
            # a293: rate-limited — caller should retry next window.
            # Don't increment errored (this isn't a real failure of
            # the lead); skip + count as a rate-limit defer instead.
            print(f"    {_yellow('⏸ rate-limited:')} {e}")
            paused += 1
            # Stop iterating once rate-limited — every subsequent
            # send in this batch will hit the same cap.
            break
        except SMTPDeliveryError as e:
            # a293: typed bounce — mark the lead so we don't keep
            # retrying a permanently-bad recipient. Persist a status
            # flag on the lead via merge_lead.
            label = "✗ bounced (permanent):" if e.permanent else "⏸ deferred (4xx):"
            print(f"    {_red(label)} SMTP {e.code}: {e.message[:80]}")
            try:
                def _mark_bounce(lead: dict, _code=e.code, _msg=e.message,
                                 _perm=e.permanent) -> dict:
                    lead["smtp_error_code"] = _code
                    lead["smtp_error_msg"] = _msg[:240]
                    if _perm:
                        lead["bounced"] = True
                        lead["email_status"] = "bounced"
                    return lead
                # a303 hotfix: a293's code referenced `db` (no import —
                # module uses `_db`) and `lead_id` (not defined — loop
                # var is `ld`). Both NameErrors got swallowed by the
                # except below, so every permanent bounce silently
                # failed to persist. The next cron tick re-sent the
                # same email to the same dead recipient — exactly the
                # loop a293 was supposed to close. Fixed names.
                await _db.merge_lead(user_id, ld.get("lead_id"), _mark_bounce)
            except Exception as _persist_err:
                print(f"    {_yellow('!')} bounce persist failed: {_persist_err}")
            errored += 1
            continue
